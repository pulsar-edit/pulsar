var $$, KeyboardLayout, KeymapManager, appendContent, buildKeydownEvent, buildKeyupEvent, debounce, fs, getFakeClock, mockProcessPlatform, nativeFs, os, path, restoreProcessPlatform, stub, temp;
...
({appendContent, stub, getFakeClock, mockProcessPlatform, restoreProcessPlatform, buildKeydownEvent, buildKeyupEvent} = require('./keymap-spec-helpers/helpers'));
...
describe("KeymapManager", function() {
  ...
  describe("::handleKeyboardEvent(event)", function() {
    ...
    describe("when the keystroke matches multiple bindings on the same element", function() {
      ...
      beforeEach(function() {
        ...
        elementB = elementA.firstChild;
        events = [];
        elementA.addEventListener('command-1', (function(e) {
          return events.push(e);
        }), false);
        elementA.addEventListener('command-2', (function(e) {
          return events.push(e);
        }), false);
        return elementA.addEventListener('command-3', (function(e) {
          return events.push(e);
        }), false);
      });
      ...
      return describe("when bindings have different priorities", function() {
        return it("dispatches the command associated with the binding which has the highest priority", function() {
          keymapManager.add("keybindings-with-super-priority", {
            ".c": {
              "ctrl-x": "command-1"
            }
          }, 2);
          keymapManager.add("keybindings-with-priority", {
            ".b.c.d": {
              "ctrl-x": "command-2"
            }
          }, 1);
          keymapManager.add("normal-keybindings", {
            ".b.c.d": {
              "ctrl-x": "command-3"
            }
          }, 0);
    ...
    describe("when the keystroke partially matches bindings", function() {
      ...
      describe("when subsequent keystrokes yield an exact match", function() {
        return it("dispatches the command associated with the matched multi-keystroke binding", function() {
          ...
          keymapManager.handleKeyboardEvent(buildKeyupEvent({
            key: 'v',
            target: editor
          }));
          keymapManager.handleKeyboardEvent(buildKeydownEvent({
      ...
      describe("when subsequent keystrokes yield no matches", function() {
        it("disables the bindings with the longest keystroke sequences and replays the queued keystrokes", function() {
          ...
          keymapManager.handleKeyboardEvent(vEvent = buildKeydownEvent({
            key: 'v',
            target: editor
          }));
          keymapManager.handleKeyboardEvent(iEvent = buildKeydownEvent({
            key: 'i',
            target: editor
          }));
          keymapManager.handleKeyboardEvent(wEvent = buildKeydownEvent({
            key: 'w',
            target: editor
          }));
          ...
          keymapManager.handleKeyboardEvent(kEvent = buildKeydownEvent({
            key: 'k',
            target: editor
          }));
          assert.equal(vEvent.defaultPrevented, true);
        ...
        return it("dispatches a text-input event for any replayed keyboard events that would have inserted characters", function() {
          ...
          keymapManager.handleKeyboardEvent(lastEvent = buildKeydownEvent({
            key: 'q',
            target: editor
          }));
    ...
    describe("when the binding specifies a keyup handler", function() {
      ...
      it("does _not_ dispatch the command when extra user-generated keydown events not specified in the binding occur between keydowns", function() {
        keymapManager.handleKeyboardEvent(buildKeydownEvent('z', {
          ctrl: true,
          target: elementA
        }));
        keymapManager.handleKeyboardEvent(buildKeyupEvent('ctrl', {
          target: elementA
        }));
        keymapManager.handleKeyboardEvent(buildKeyupEvent('ctrl', {
          target: elementA
        ...
        keymapManager.handleKeyboardEvent(buildKeydownEvent('d', {
          target: elementA
        }));
        keymapManager.handleKeyboardEvent(buildKeydownEvent('e', {
          target: elementA
        }));
        keymapManager.handleKeyboardEvent(buildKeydownEvent('f', {
          target: elementA
        }));
  ...
  describe("::build(source, bindings)", function() {
    ...
    it("rejects bindings with an invalid selector and logs a warning to the console", function() {
      stub(console, 'warn');
      assert.lengthOf(keymapManager.build('test', {
        '<>': {
          'shift-a': 'a'
        }
      }), 0);
  ...
  describe("::add(source, bindings)", function() {
    ...
    return it("returns a disposable allowing the added bindings to be removed", function() {
      ...
      assert.equal(keymapManager.findKeyBindings({
        command: 'x'
      }).length, 1);
      assert.equal(keymapManager.findKeyBindings({
        command: 'y'
  ...
  describe("::keystrokeForKeyboardEvent(event)", function() {
    describe("when no extra modifiers are pressed", function() {
      return it("returns a string that identifies the unmodified keystroke", function() {
        ...
        return assert.equal(keymapManager.keystrokeForKeyboardEvent(buildKeydownEvent({
          key: ' ',
          code: 'Space'
        })), 'space');
      });
    ...
    return describe("when custom keystroke resolvers are installed", function() {
      return it("resolves to the keystroke string of the most recently-installed resolver returning a defined value", function() {
        var currentKeymap, currentLayoutName, disposable1, disposable2, disposable3, expectedKeystroke, keydownEvent;
        mockProcessPlatform('darwin');
        currentKeymap = require('./keymap-spec-helpers/keymaps/mac-swiss-german');
        ...
        disposable1 = keymapManager.addKeystrokeResolver(function({keystroke, event, layoutName, keymap}) {
        ...
        return assert.equal(keymapManager.keystrokeForKeyboardEvent(keydownEvent), expectedKeystroke);
      });
    });
  });
  describe("::findKeyBindings({command, target, keystrokes})", function() {
    ...
    describe("when only passed a command", function() {
      return it("returns all bindings that dispatch the given command", function() {
        var keystrokes;
        keystrokes = keymapManager.findKeyBindings({
          command: 'x'
        }).map(function(b) {
          return b.keystrokes;
        }).sort();
      ...
      return describe("if called with watch: true", function() {
        ...
        describe("when the file is changed", function() {
          ...
          it("reloads the file's key bindings and notifies ::onDidReloadKeymap observers with the keymap path", function(done) {
            ...
            fs.writeFileSync(keymapFilePath, `'.a': 'ctrl-a': 'y'
'.b': 'ctrl-b': 'z'`);
          });
...
'.b': 'ctrl-b': 'z'`);
  ...
  return describe("events", function() {
    ...
    it("emits `matched-partially` when a key binding partially matches an event", function() {
      ...
      assert.deepEqual(partiallyMatchedBindings.map(function({command}) {
        return command;
      }), ['command-1', 'command-2']);
      return assert.equal(keyboardEventTarget, document.body);
    });
