...
describe("TextBuffer", function() {
  ...
  describe("::setTextInRange(range, text)", function() {
    ...
    describe("when the normalizeLineEndings argument is true (the default)", function() {
      ...
      describe("when the range's start row has no line ending (because it's the last line of the buffer)", function() {
        ...
        describe("when the buffer contains newlines", () => it("normalizes inserted line endings to match the line ending of the penultimate row", function() {
          expect(buffer.lineEndingForRow(1)).toBe('\r\n');
    ...
    describe("when the normalizeLineEndings argument is false", () => it("honors the newlines in the inserted text", function() {
  ...
  describe("::createMarkerSnapshot", function() {
    ...
    describe("when selectionsMarkerLayer is passed", () => it("skips snapshotting of other 'selection' role marker layers", function() {
  ...
  describe("selective snapshotting and restoration on transact/undo/redo for selections marker layer", function() {
    ...
    describe("selections marker layer's selective snapshotting on createCheckpoint, groupChangesSinceCheckpoint", () => it("skips snapshotting of other marker layers with the same role as the selectionsMarkerLayer", function() {
...
`
  ...
  describe("serialization", function() {
    ...
    it("serializes / deserializes the buffer's persistent custom marker layers", function(done) {
      ...
      TextBuffer.deserialize(JSON.parse(JSON.stringify(bufferA.serialize()))).then(function(bufferB) {
    ...
    describe("when the serialized buffer was unsaved and had no path", () => it("restores the previous unsaved state of the buffer", function(done) {
      buffer = new TextBuffer();
      buffer.setText("abc");
  ...
  describe("::rangeForRow(row, includeNewline)", function() {
    ...
    describe("if the given row is out of range", () => it("returns the range of the nearest valid row", function() {
  ...
  xdescribe("::onWillThrowWatchError", () => it("notifies observers when the file has a watch error", function() {
    const filePath = temp.openSync('atom').path;
    fs.writeFileSync(filePath, '');
  ...
  describe("::setTextInRange(range, string)", function() {
    ...
    describe("when used to insert (called with an empty range and a non-empty string)", function() {
      ...
      describe("when the given string has newlines", () => it("inserts the lines at the location of the given range", function() {
        const range = [[3, 4], [3, 4]];
    ...
    describe("when used to remove (called with a non-empty range and an empty string)", function() {
      ...
      describe("when the range spans more than 2 lines", () => it("removes the characters within the range, joining the first and last line and removing the lines in-between", function() {
  ...
  describe("::setText(text)", function() {
    ...
    describe("with windows newlines", () => it("changes the entire contents of the buffer", function() {
  ...
  describe("::getTextInRange(range)", function() {
    ...
    describe("when the range ends after the end of the buffer", () => it("clips the range to the end of the buffer", () => expect(buffer.getTextInRange([[12], [13, Infinity]])).toBe(buffer.lineForRow(12))));
  });
  ...
  describe("::scanInRange(range, regex, fn)", function() {
    ...
    describe("when the last regex match exceeds the end of the range", function() {
      ...
      describe("when the portion of the match within the range does not matches the regex", () => it("does not call the iterator with the truncated match", function() {
        const matches = [];
        const ranges = [];
    ...
    it("returns the same results as a regex match on a regular string", function() {
      ...
      while (i < 20) {
        ...
        var expectedMatches = (left = buffer.getTextInRange(range).match(regex)) != null ? left : [];
  ...
  describe("::backwardsScanInRange(range, regex, fn)", function() {
    ...
    describe("when the iterator calls the 'replace' control function with a replacement string", () => it("replaces each occurrence of the regex match with the string", function() {
      ...
      buffer.backwardsScanInRange(/cu(rr)ent/g, [[4, 0], [6, 59]], function({range, replace}) {
        ranges.push(range);
        if (!range.start.isEqual([6, 6])) { replace("foo"); }
      });
  ...
  describe("when called with a random range", () => {
    it("returns the same results as ::scanInRange, but in the opposite order", () => {
      for (let i = 1; i < 50; i++) {
        ...
        var [startRow, endRow] = [random(buffer.getLineCount()), random(buffer.getLineCount())].sort();
        var startColumn = random(buffer.lineForRow(startRow).length);
        var endColumn = random(buffer.lineForRow(endRow).length);
        var range = [[startRow, startColumn], [endRow, endColumn]];
  ...
  describe("::characterIndexForPosition(position)", function() {
    ...
    describe("when the buffer contains crlf line endings", () => it("returns the total number of characters that precede the given position", function() {
  ...
  describe("::positionForCharacterIndex(position)", function() {
    ...
    describe("when the buffer contains crlf line endings", () => it("returns the position based on character index", function() {
      buffer.setText("line1\r\nline2\nline3\r\nline4");
  ...
  describe("::hasAstral()", function() {
    ...
    it("returns false for buffers that do not contain surrogate pairs", () => expect(new TextBuffer('nope').hasAstral()).toBeFalsy());
  });
  ...
  describe("line ending support", function() {
    ...
    describe("when inserting text with multiple lines", function() {
      ...
      describe("when the current line has no line ending (because it's the last line of the buffer)", function() {
        ...
        describe("when the buffer contains a preceding line", () => it("uses the line ending of the preceding line", function() {
          buffer.setText("\ninitialtext");
          buffer.append("hello\n1\r\n2\n");
          expect(buffer.getText()).toBe("\ninitialtexthello\n1\n2\n");
        }));
