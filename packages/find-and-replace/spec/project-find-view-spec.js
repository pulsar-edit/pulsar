...
for (const ripgrep of [false, true]) {
  describe(`ProjectFindView (ripgrep=${ripgrep})`, () => {
    ...
    let activationPromise, editor, editorElement, findView, projectFindView, workspaceElement;
    ...
    beforeEach(() => {
      ...
      activationPromise = atom.packages.activatePackage("find-and-replace").then(function ({
        mainModule,
      }) {
    ...
    describe("when project-find:show is triggered", () => {
      it("attaches ProjectFindView to the root view", async () => {
        ...
        expect(projectFindView.findEditor.getSelectedBufferRange()).toEqual([
          [0, 0],
          [0, 5],
        ]);
      });
      ...
      describe("when the ProjectFindView is already attached", () => {
        beforeEach(async () => {
          ...
          projectFindView.findEditor.setText("items");
          projectFindView.findEditor.setSelectedBufferRange([
            [0, 0],
            [0, 0],
          ]);
        });
    ...
    describe("when project-find:show-in-current-directory is triggered", () => {
      ...
      it("populates the pathsEditor when triggered on a file", async () => {
        atom.commands.dispatch(
          nested.querySelector(".file .name"),
          "project-find:show-in-current-directory",
        );
        await activationPromise;
      ...
      describe("when there are multiple root directories", async () => {
        ...
        it("includes the basename of the containing root directory in the paths-editor", async () => {
          ...
          expect(getAtomPanel()).toBeVisible();
          expect(projectFindView.pathsEditor.getText()).toBe(
            path.join(path.basename(projectPath), "nested"),
          );
        });
      });
    });
    ...
    describe("finding", () => {
      ...
      describe("when the find string contains an escaped char", () => {
        ...
        describe("when regex search is enabled", () => {
          it("finds a literal tab character", async () => {
            ...
            await waitForCondition(() =>
              resultsView.refs.listView.element.querySelector(".match-row"),
            );
            expect(resultsView.element).toBeVisible();
            expect(resultsView.refs.listView.element.querySelectorAll(".match-row")).toHaveLength(
              2,
            );
          });
        });
        ...
        describe("when regex search is disabled", () => {
          ...
          it("finds a backslash", async () => {
            ...
            expect(
              resultsView.refs.listView.element.querySelectorAll(".match.highlight-info"),
            ).toHaveLength(3);
          });
      ...
      describe("splitting into a second pane", () => {
        ...
        it("can be duplicated on the right", async () => {
          ...
          await waitForCondition(() =>
            resultsView1.refs.listView.element.querySelector(".match-row"),
          );
          ...
          expect(resultsPaneView2.querySelector(".preview-count").innerHTML).toEqual(
            resultsPaneView1.querySelector(".preview-count").innerHTML,
          );
        });
      ...
      describe("description label", () => {
        ...
        it("indicates that it's searching, then shows the results", async () => {
          ...
          expect(projectFindView.refs.descriptionLabel.textContent).toContain(
            "13 results found in 2 files",
          );
          atom.commands.dispatch(projectFindView.element, "core:confirm");
          expect(projectFindView.refs.descriptionLabel.textContent).toContain(
            "13 results found in 2 files",
      ...
      describe("when project-find:confirm is triggered", () => {
        it("displays the results and no errors", async () => {
          ...
          await waitForCondition(() => {
            return (
              (getResultsView()?.refs.listView.element.querySelectorAll(".match-row").length ??
                0) >= 11
            );
          });
      ...
      describe("when core:confirm is triggered", () => {
        ...
        describe("when results exist", () => {
          ...
          it("displays the results and no errors", async () => {
            ...
            expect(resultsPaneView.refs.previewCount.textContent).toBe(
              "13 results found in 2 files for items",
            );
          ...
          it("updates the results list when a buffer changes", async () => {
            ...
            expect(listView.element.querySelectorAll(".path-row")[1].parentElement).toHaveClass(
              "selected",
            );
            ...
            await waitForCondition(
              () =>
                resultsPaneView.refs.previewCount.textContent ===
                "8 results found in 2 files for items",
            );
            expect(listView.element.querySelectorAll(".path-row")[1].parentElement).toHaveClass(
              "selected",
            );
            ...
            await waitForCondition(() => editor.getSelectedText() === "items");
            ...
            await waitForCondition(
              () =>
                resultsPaneView.refs.previewCount.textContent ===
                "7 results found in 1 file for items",
            );
            expect(resultsPaneView.refs.previewCount.textContent).toEqual(
              "7 results found in 1 file for items",
            );
          });
          ...
          it("doesn't update the results list when a buffer outside the project changes", async () => {
            ...
            expect(resultsView.refs.listView.element.querySelectorAll(".list-item")).toHaveLength(
              13,
            );
            expect(resultsPaneView.refs.previewCount.textContent).toBe(
              "13 results found in 2 files for items",
            ...
            expect(
              resultsView.refs.listView.element.querySelectorAll(".list-nested-item")[1],
    ...
    describe("replacing", () => {
      ...
      beforeEach(async () => {
        ...
        fs.writeFileSync(
          sampleCoffee,
          fs.readFileSync(require.resolve("./fixtures/sample.coffee")),
        );
        fs.writeFileSync(
          sampleJs,
          fs.readFileSync(require.resolve("./fixtures/project/sample.js")),
        );
      ...
      describe("when the replace button is pressed", () => {
        ...
        describe("when there are search results after a replace", () => {
          it("runs the search after the replace", async () => {
            ...
            expect(projectFindView.errorMessages).not.toBeVisible();
            expect(getExistingResultsPane().refs.previewCount.textContent).toContain(
              "13 results found in 2 files for items",
            );
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
              "Replaced items with items-123 13 times in 2 files",
            );

            projectFindView.replaceEditor.setText("cats");
            advanceClock(projectFindView.replaceEditor.getBuffer().stoppedChangingDelay);
            expect(projectFindView.refs.descriptionLabel.textContent).not.toContain(
              "Replaced items",
            );
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
              "13 results found in 2 files for items",
      ...
      describe("when the project-find:replace-all is triggered", () => {
        ...
        describe("when a search with no results has been run", () => {
          ...
          it("doesnt replace anything", () => {
            ...
            expect(atom.beep).toHaveBeenCalled();
            expect(
              projectFindView.refs.descriptionLabel.textContent.replace(/( {2})/g, " "),
        ...
        describe("when a search with results has been run", () => {
          ...
          it("replaces all the matches and updates the results view", async () => {
            ...
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
              "Replaced items with sunshine 13 times in 2 files",
            );
        ...
        describe("when the find field contains a ^ or a $ and the regex option is enabled", () => {
          it("correctly replaces all matches", async () => {
            ...
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
              "Replaced ;$ with sunshine 9 times in 2 files",
            );
      ...
      describe("when there is an error replacing", () => {
        ...
        it("displays the errors in the results pane", async () => {
          let errorList;
          spyOn(atom.workspace, "replace").andCallFake(
            async (regex, replacement, paths, callback) => {
    ...
    describe("panel focus", () => {
      ...
      it("moves focus between editors with find-and-replace:focus-next", () => {
        ...
        atom.commands.dispatch(
          projectFindView.replaceEditor.element,
    ...
    describe("when language-javascript is active", () => {
      ...
      it("uses the regexp grammar when regex-mode is loaded from configuration", async () => {
        ...
        expect(projectFindView.replaceEditor.getGrammar().scopeName).toBe(
