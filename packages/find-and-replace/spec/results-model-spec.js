...
describe("ResultsModel", () => {
  ...
  describe("searching for a pattern", () => {
    it("populates the model with all the results, and updates in response to changes in the buffer", async () => {
      ...
      expect(result.matches[0].trailingContextLines[0]).toBe(
        "    if (items.length <= 1) return items;",
      );
      expect(result.matches[0].trailingContextLines[1]).toBe(
    ...
    describe("when the project has a single root", () => {
      beforeEach(() => {
        atom.project.setPaths([path.join(__dirname, "fixtures/another-project-root")]);
        resultsModel = new ResultsModel(new FindOptions({ pathsPattern: "sub/sample.js" }));
    ...
    describe("when a file is modified before the search starts", () => {
      ...
      it("still appears in search results", async () => {
        ...
        expect(resultsModel.getResult(modifiedPath)).not.toBe(undefined);
      });
    });
    ...
    describe("when a file is modified after the search finishes", () => {
      ...
      it("is added to the results once it matches (with no path pattern)", async () => {
        jasmine.useRealClock();
        await resultsModel.search(" var quicksort =", "", "");
        expect(resultsModel.getResult(modifiedPath)).toBe(undefined);
        modifyEditor();
      ...
      it("has its path inclusions and exclusions assessed properly", async () => {
        ...
        await resultsModel.search(
          " var quicksort =",
          "**/sample.js, another-project-root/foo.js, project/sample.js",
          "",
        );
        expect(resultsModel.getResult(modifiedPath)).toBe(undefined);
        modifyEditor();
    ...
    describe("when the project has multiple roots", () => {
      ...
      it("should allow us to specify multiple path inclusions that point to different roots", async () => {
        await resultsModel.search(
          "quicksort =",
          "another-project-root/sub/sample.js, project/sub/sample.js",
          "",
        );
      ...
      describe("and a file is already modified when search begins", () => {
        ...
        it("has its path inclusions and exclusions assessed properly", async () => {
          ...
          await resultsModel.search(
            "quicksort =",
            "**/sample.js, !another-project-root/sample.js, project/sample.js",
            "",
          );
          expect(resultsModel.getResult(modifiedPath)).not.toBe(undefined);
        });
