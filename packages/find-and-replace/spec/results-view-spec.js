...
describe("ResultsView", () => {
  ...
  beforeEach(async () => {
    ...
    let activationPromise = atom.packages.activatePackage("find-and-replace").then(function ({
      mainModule,
    }) {
  ...
  describe("when the result is for a long line", () => {
    it("renders the context around the match", async () => {
      ...
      expect(resultsView.refs.listView.element.querySelector(".path-name").textContent).toBe(
        "one-long-line.coffee",
      );
      expect(resultsView.refs.listView.element.querySelectorAll(".preview").length).toBe(1);
      expect(resultsView.refs.listView.element.querySelector(".preview").textContent).toBe(
        "test test test test test test test test test test test a b c d e f g h i j k l abcdefghijklmnopqrstuvwxyz",
      );
  ...
  describe("when there are multiple project paths", () => {
    ...
    it("includes the basename of the project path that contains the match", async () => {
      ...
      resultsView = getResultsView();
      expect(resultsView.refs.listView.element.querySelector(".path-name").textContent).toBe(
        path.join("project", "one-long-line.coffee"),
      );
    });
  });
  ...
  describe("rendering replacement text", () => {
    ...
    it("renders the replacement when changing the text in the replacement field", async () => {
      ...
      resultsView = getResultsView();
      expect(resultsView.refs.listView.element.querySelector(".match").textContent).toBe("ghijkl");
      expect(resultsView.refs.listView.element.querySelector(".match")).toHaveClass(
        "highlight-info",
      );
      expect(resultsView.refs.listView.element.querySelector(".replacement").textContent).toBe("");
      ...
      await waitForCondition(() =>
        resultsView.refs.listView.element
          .querySelector(".match")
          ?.classList.contains("highlight-error"),
      );
      expect(resultsView.refs.listView.element.querySelector(".match").textContent).toBe("ghijkl");
      expect(resultsView.refs.listView.element.querySelector(".match")).toHaveClass(
        "highlight-error",
      ...
      await waitForCondition(() =>
        resultsView.refs.listView.element
          .querySelector(".match")
          ?.classList.contains("highlight-info"),
      );
      expect(resultsView.refs.listView.element.querySelector(".match").textContent).toBe("ghijkl");
      expect(resultsView.refs.listView.element.querySelector(".match")).toHaveClass(
        "highlight-info",
  ...
  describe("core:page-up and core:page-down", () => {
    ...
    it("selects the first result on the next page when core:page-down is triggered", async () => {
      const { listView } = resultsView.refs;
      expect(listView.element.querySelectorAll(".path-row").length).not.toBeGreaterThan(
        resultsView.model.getPathCount(),
      );
      expect(listView.element.querySelectorAll(".match-row").length).not.toBeGreaterThan(
        resultsView.model.getMatchCount(),
      );
  ...
  describe("core:move-to-top and core:move-to-bottom", () => {
    ...
    it("selects the first/last item when core:move-to-top/move-to-bottom is triggered", async () => {
      const { listView } = resultsView.refs;
      expect(listView.element.querySelectorAll("li").length).toBeLessThan(
        resultsView.model.getPathCount() + resultsView.model.getMatchCount(),
      );
    ...
    it("selects the path when when core:move-to-bottom is triggered and last item is collapsed", async () => {
      ...
      await waitForCondition(() =>
        resultsView.refs.listView.element
          .querySelector(".path-row")
          .parentElement.classList.contains("selected"),
      );

      expect(
        _.last(resultsView.refs.listView.element.querySelectorAll(".path-row")).parentElement,
  ...
  describe("expanding and collapsing results", () => {
    ...
    it("re-expands all results when running a new search", async () => {
      ...
      atom.commands.dispatch(projectFindView.element, "core:confirm");
      await waitForCondition(() => !resultsView.element.querySelector(".collapsed"));
    ...
    it("preserves the collapsed state of the right files when results are removed", async () => {
      ...
      const matchedPaths =
        resultsView.refs.listView.element.querySelectorAll(".path.list-nested-item");
  ...
  describe("arrowing through the list", () => {
    it("arrows through the entire list without selecting paths and overshooting the boundaries", async () => {
      ...
      let { length: resultCount } =
        resultsView.refs.listView.element.querySelectorAll(".match-row");
    ...
    describe("when there are a list of items", () => {
      ...
      it("collapses all results if collapse All button is pressed", async () => {
        ...
        for (let item of Array.from(
          resultsView.refs.listView.element.querySelectorAll(".path-row"),
        )) {
      ...
      describe("when there are collapsed results", () => {
        it("moves to the correct prev/next result when a path is selected", async () => {
          ...
          expect(resultsView.refs.listView.element.querySelectorAll(".match-row")[0]).toHaveClass(
            "selected",
          );

          await resultsView.moveUp();
          expect(
            resultsView.refs.listView.element.querySelectorAll(".path-row")[1].parentElement,
          ).toHaveClass("selected");

          await resultsView.moveUp();
          expect(
            resultsView.refs.listView.element.querySelectorAll(".path-row")[0].parentElement,
          ).toHaveClass("selected");
  ...
  describe("icon services", () => {
    describe("atom.file-icons", () => {
      ...
      it("allows a service provider to change the handler", async () => {
        ...
        resultsView = getResultsView();
        let fileIconClasses = Array.from(
          resultsView.refs.listView.element.querySelectorAll(".path-row .icon"),
        ).map((el) => el.className);
    ...
    describe("file-icons.element-icons", () => {
      ...
      it("uses the element-icon service if available", async () => {
        ...
        const provider = (element, path) => {
          ...
          const classes = path.endsWith("one-long-line.coffee") ? ["foo", "bar"] : ["baz", "qlux"];
        ...
        disposable = atom.packages.serviceHub.provide(
  ...
  describe("updating the search while viewing results", () => {
    it("resets the results message", async () => {
      ...
      await waitForCondition(() => resultsPane?.refs?.previewCount?.textContent.match(/3 files/));
      expect(resultsPane.refs.previewCount.textContent).toContain("3 files");

      projectFindView.findEditor.setText("");
      atom.commands.dispatch(projectFindView.element, "core:confirm");
      await waitForCondition(() => resultsPane.refs.previewCount.textContent.match(/Project/));
  ...
  describe("search result context lines", () => {
    ...
    function getFirstMatchRows(resultsView) {
      ...
      const rowCount = resultsView.resultRowGroups[0].rows.filter(
        (row) => row.data.matchLineNumber === resultsView.resultRows[1].data.matchLineNumber,
      ).length;
    ...
    it("shows no context lines", async () => {
      ...
      expect(lineNodes[0].querySelector(".preview").textContent).toBe(
    ...
    it("shows 1 leading context line, 1 trailing context line", async () => {
      ...
      */
      ...
      expect(lineNodes[1].querySelector(".preview").textContent).toBe(
        "    return items if items.length <= 1",
      );
    ...
    it("maintains selected result when adding and removing results", async () => {
      {
        ...
        expect(matchRows[2].querySelector(".preview").textContent).toBe(
      ...
      // check that the same match is still selected
      {
        ...
        expect(matchRows[0].querySelector(".preview").textContent).toBe(
          "      current < pivot ? left.push(current) : right.push(current);",
        );
