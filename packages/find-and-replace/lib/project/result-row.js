...
class ResultRowGroup {
  ...
  generateRows(findOptions) {
    ...
    for (const match of this.result.matches) {
      ...
      if (prevMatch) {
        ...
        rowArrays.push(
          trailingContextLines
            .slice(0, trailCount)
            .map((line, i) => new TrailingContextRow(this, line, false, prevLineNumber, i + 1)),
        );
      ...
      rowArrays.push(
        leadingContextLines
          .slice(leadingContextLines.length - leadCount)
          .map((line, i) => new LeadingContextRow(this, line, false, lineNumber, leadCount - i)),
      );
    ...
    rowArrays.push(
      trailingContextLines
        .slice(0, trailingContextLineCount)
        .map((line, i) => new TrailingContextRow(this, line, false, prevLineNumber, i + 1)),
    );
    ...
    for (const row of this.rows) {
      row.data.separator =
        prevRow &&
        row.data.lineNumber != null &&
        prevRow.data.lineNumber != null &&
