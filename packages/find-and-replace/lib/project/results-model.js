...
module.exports = class ResultsModel {
  ...
  async search(findPattern, pathsPattern, replacePattern, options = {}) {
    ...
    const leadingContextLineCount = atom.config.get(
      "find-and-replace.searchContextLineCountBefore",
    );
    const trailingContextLineCount = atom.config.get(
  ...
  replace(pathsPattern, replacePattern, replacementPaths) {
    ...
    const promise = atom.workspace.replace(
      this.regex,
      replacePattern,
      replacementPaths,
      (result, error) => {
  ...
  onContentsModified(editor) {
    if (!this.active || !this.regex || !editor.getPath()) {
      return;
    }
    ...
    editor.scan(this.regex, { leadingContextLineCount, trailingContextLineCount }, (match) =>
      matches.push(match),
    );
