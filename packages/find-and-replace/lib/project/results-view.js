...
module.exports = class ResultsView {
  constructor({ model }) {
    ...
    this.resultRowGroups.sort((group1, group2) =>
      filepathComp(group1.result.filePath, group2.result.filePath),
    );
  ...
  getGroupCountBefore(filePath) {
    const res = binarySearch(this.resultRowGroups, filePath, (rowGroup, needle) =>
      filepathComp(rowGroup.result.filePath, needle),
    );
  ...
  async confirmResult({ pending } = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await atom.workspace.open(selectedRow.group.result.filePath, {
        ...
        split:
          reverseDirections[
            atom.config.get("find-and-replace.projectSearchResultsPaneSplitDirection")
          ],
      });
