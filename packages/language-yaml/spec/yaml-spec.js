  ...
  describe("strings", function () {
    describe("double quoted", function () {
      it("parses escaped quotes", function () {
        ...
        expect(tokens[0]).toEqual({
          value: "key",
          scopes: ["source.yaml", "entity.name.tag.yaml"],
        });
        expect(tokens[1]).toEqual({
          value: ":",
          scopes: ["source.yaml", "punctuation.separator.key-value.yaml"],
        });
      ...
      it("parses invalid escape sequences", function () {
        ...
        expect(tokens[1]).toEqual({
          value: "\\uqerw",
          scopes: ["source.yaml", "string.quoted.double.yaml", "invalid.illegal.escape.yaml"],
        });
        expect(tokens[2]).toEqual({
          value: "w",
          scopes: ["source.yaml", "string.quoted.double.yaml"],
...
`);
        expect(lines[0][0]).toEqual({
          value: "-",
          scopes: ["source.yaml", "punctuation.definition.entry.yaml"],
        });
...
`);
        ...
        expect(lines[1][0]).toEqual({
          value: "  # this is not a legit comment",
          scopes: ["source.yaml", "string.unquoted.block.yaml"],
        });
        expect(lines[2][0]).toEqual({
          value: "  unquoted block",
          scopes: ["source.yaml", "string.unquoted.block.yaml"],
        });
        expect(lines[3][0]).toEqual({
...
`);
    ...
    it("does not confuse keys and strings", function () {
      ...
      expect(tokens[3]).toEqual({
        value: "Section 2.4: 3, 6abc, 12ab, 30, 32a",
        scopes: ["source.yaml", "string.quoted.single.yaml"],
      });
    });
  });
  ...
  it("parses the non-specific tag indicator before values", function () {
    ...
    expect(tokens[3]).toEqual({
      value: "!",
      scopes: ["source.yaml", "punctuation.definition.tag.non-specific.yaml"],
    });
    expect(tokens[5]).toEqual({
      value: "'",
      scopes: [
...
`);
    ...
    expect(lines[2][4]).toEqual({
      value: "3",
      scopes: ["source.yaml", "constant.numeric.integer.yaml"],
    });
...
`);
    ...
    expect(lines[0][1]).toEqual({
      value: "G@role:deployer",
      scopes: ["source.yaml", "string.quoted.single.yaml", "entity.name.tag.yaml"],
    });
    expect(lines[0][2]).toEqual({
      value: "'",
      scopes: [
...
`);
    ...
    expect(lines[0][1]).toEqual({
      value: "G@role:deployer",
      scopes: ["source.yaml", "string.quoted.double.yaml", "entity.name.tag.yaml"],
    });
    expect(lines[0][2]).toEqual({
      value: '"',
      scopes: [
...
`);
    ...
    expect(lines[1][1]).toEqual({
      value: "This should still be a string ",
      scopes: ["source.yaml", "string.unquoted.yaml"],
    });
    expect(lines[1][2]).toEqual({
      value: "#",
...
`);
    ...
    expect(lines[1][0]).toEqual({
      value: "  This should still be a string # not a comment!",
      scopes: ["source.yaml", "string.unquoted.block.yaml"],
    });
    expect(lines[3][0]).toEqual({
      value: "  # Guess what this is",
      scopes: ["source.yaml", "string.unquoted.block.yaml"],
    });
    expect(lines[5][0]).toEqual({
      value: "#",
  ...
  it("does not confuse keys and comments", function () {
    const { tokens } = grammar.tokenizeLine(
      "- Entry 2 # This colon breaks syntax highlighting: see?",
    );
    expect(tokens[0]).toEqual({
      value: "-",
      scopes: ["source.yaml", "punctuation.definition.entry.yaml"],
    ...
    expect(tokens[4]).toEqual({
      value: " This colon breaks syntax highlighting: see?",
      scopes: ["source.yaml", "comment.line.number-sign.yaml"],
    });
  });
...
`);
  ...
  it("parses the merge-key tag", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "<<",
      scopes: ["source.yaml", "entity.name.tag.merge.yaml"],
    });
    expect(tokens[1]).toEqual({
      value: ":",
      scopes: ["source.yaml", "punctuation.separator.key-value.yaml"],
    ...
    expect(tokens[3]).toEqual({
      value: "*",
      scopes: ["source.yaml", "variable.other.yaml", "punctuation.definition.variable.yaml"],
    });
    expect(tokens[4]).toEqual({
      value: "variable",
      scopes: ["source.yaml", "variable.other.yaml"],
    });
  ...
  it("parses local tags", function () {
    ...
    expect(tokens[3]).toEqual({
      ...
      scopes: [
        "source.yaml",
        "keyword.other.tag.local.yaml",
        "punctuation.definition.tag.local.yaml",
      ],
    });
    expect(tokens[4]).toEqual({
      value: "something",
      scopes: ["source.yaml", "keyword.other.tag.local.yaml"],
    });
    expect(tokens[6]).toEqual({
      value: ">",
      scopes: ["source.yaml", "string.unquoted.block.yaml"],
  ...
  it("parses the !!omap directive", function () {
    ...
    expect(tokens[3]).toEqual({
      value: "!!",
      scopes: ["source.yaml", "keyword.other.omap.yaml", "punctuation.definition.tag.omap.yaml"],
    });
    expect(tokens[4]).toEqual({
      value: "omap",
      scopes: ["source.yaml", "keyword.other.omap.yaml"],
    });
  ...
  it("parses dates in YYYY-MM-DD format", function () {
    ...
    expect(tokens[5]).toEqual({
      value: "2001-01-01",
      scopes: ["source.yaml", "constant.other.date.yaml"],
    });
...
`);
  ...
  describe("numbers", function () {
    ...
    it("parses octals", function () {
      ...
      expect(tokens[3]).toEqual({
        value: "0o664",
        scopes: ["source.yaml", "constant.numeric.octal.yaml"],
      });
    });
    ...
    it("parses hexadecimals", function () {
      ...
      expect(tokens[3]).toEqual({
        value: "0x726Fa",
        scopes: ["source.yaml", "constant.numeric.hexadecimal.yaml"],
      });
    });
    ...
    it("parses floats", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "0.7e-9001",
        scopes: ["source.yaml", "constant.numeric.float.yaml"],
      });
...
`);
  ...
  describe("constants", function () {
    it("tokenizes true, false, and null as constants", function () {
      ...
      expect(tokens[3]).toEqual({
        value: "true",
        scopes: ["source.yaml", "constant.language.yaml"],
      });
  ...
  describe("structures", function () {
    it("tokenizes directives end markers", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "---",
        scopes: ["source.yaml", "punctuation.definition.directives.end.yaml"],
      });
    ...
    it("tokenizes document end markers", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "...",
        scopes: ["source.yaml", "punctuation.definition.document.end.yaml"],
      });
    });
...
`);
  ...
  describe("tabs", () =>
    it("marks them as invalid", function () {
      const { tokens } = grammar.tokenizeLine("\t\ttabs:");
      expect(tokens[0]).toEqual({
        value: "\t\t",
        scopes: ["source.yaml", "invalid.illegal.whitespace.yaml"],
      });
    }));
});
