...
describe("PackageUpdatesStatusView", function () {
  ...
  const outdatedPackage1 = { name: "out-dated-1" };
  const outdatedPackage2 = { name: "out-dated-2" };
  const installedPackage = { name: "user-package" };

  beforeEach(function () {
    spyOn(PackageManager.prototype, "loadCompatiblePackageVersion").andCallFake(function () {});
    spyOn(PackageManager.prototype, "getInstalled").andCallFake(() =>
      Promise.resolve([installedPackage]),
    );
    spyOn(PackageManager.prototype, "getOutdated").andCallFake(() =>
      Promise.resolve([outdatedPackage1, outdatedPackage2]),
    );
    ...
    runs(function () {
      ...
      return (packageManager =
        PackageUpdatesStatusView.prototype.initialize.mostRecentCall.args[1]);
    });
  });

  describe("when packages are outdated", () =>
    it("adds a tile to the status bar", () =>
      expect(document.querySelector("status-bar .package-updates-status-view").textContent).toBe(
        "2 updates",
      )));
  ...
  describe("when multiple events are happening at the same time", () =>
    it("updates the tile", function () {
      ...
      expect(document.querySelector("status-bar .package-updates-status-view").textContent).toBe(
        "1/3 updating (1 failed)",
      );
    }));

  describe("when there are no more updates", () =>
    it("destroys the tile", function () {
      packageManager.emitPackageEvent("updated", outdatedPackage1);
  ...
  describe("when a new update becomes available and the tile is destroyed", () =>
    it("recreates the tile", function () {
      packageManager.emitPackageEvent("updated", outdatedPackage1);
      packageManager.emitPackageEvent("updated", outdatedPackage2);
      packageManager.emitPackageEvent("update-available", installedPackage);
      expect(document.querySelector("status-bar .package-updates-status-view")).toExist();
  ...
  describe("when updates are checked for multiple times and no new updates are available", () =>
    it("does not keep updating the tile", function () {
      ...
      packageManager.emitPackageEvent("update-available", {
        name: "out-dated-1",
        date: Date.now(),
      });
      expect(document.querySelector("status-bar .package-updates-status-view").textContent).toBe(
