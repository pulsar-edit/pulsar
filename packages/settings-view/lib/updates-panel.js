...
export default class UpdatesPanel {
  constructor(settingsView, packageManager) {
    ...
    this.disposables.add(
      this.packageManager.on("package-updating theme-updating", ({ pack, error }) => {
    ...
    this.disposables.add(
      this.packageManager.on(
        "package-updated theme-updated package-update-failed theme-update-failed",
        ({ pack, error }) => {
          if (error != null) {
      ...
      <div tabIndex="0" className="panels-item">
        <section className="section packages">
          <div className="section-container updates-container">
            ...
            <div ref="noUpdatesMessage" className="alert alert-info icon icon-heart">
  ...
  async updateAll() {
    ...
    const concurrency =
      atom.config.get("settings-view.packageUpdateConcurrency") > 0
    ...
    if (successfulUpdatesCount > 0) {
      const message = `Restart Lumine to complete the update of ${successfulUpdatesCount} ${pluralize("package", successfulUpdatesCount)}:`;
      let detail = "";
