...
export default class PackageUpdatesStatusView {
  initialize(statusBar, packageManager, updates) {
    ...
    this.disposables.add(
      packageManager.on("package-update-available theme-update-available", ({ pack, error }) => {
        this.onPackageUpdateAvailable(pack);
      }),
    );
    this.disposables.add(
      packageManager.on("package-updating theme-updating", ({ pack, error }) => {
        this.onPackageUpdating(pack);
      }),
    );
    this.disposables.add(
      packageManager.on(
        "package-updated theme-updated package-uninstalled theme-uninstalled",
        ({ pack, error }) => {
    ...
    this.disposables.add(
      packageManager.on("package-update-failed theme-update-failed", ({ pack, error }) => {
