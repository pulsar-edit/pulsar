describe("Python grammar", () => {
  ...
  it("tokenizes `yield`", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "yield",
      scopes: ["source.python", "keyword.control.statement.python"],
    });
  });
  ...
  it("tokenizes multi-line strings", () => {
    ...
    expect(tokens[1][0].scopes).toEqual([
      "source.python",
      "string.quoted.double.single-line.python",
    ]);

    expect(tokens[1][1].value).toBe('"');
    expect(tokens[1][1].scopes).toEqual([
      "source.python",
      "string.quoted.double.single-line.python",
  ...
  it("terminates a single-quoted raw string containing opening parenthesis at closing quote", () => {
    ...
    expect(tokens[0][4].scopes).toEqual([
      "source.python",
      "string.quoted.single.single-line.raw-regex.python",
      "punctuation.definition.string.end.python",
    ]);
    ...
    expect(tokens[0][6].scopes).toEqual([
      "source.python",
  ...
  it("terminates referencing an item in a list variable after a sequence of a closing and opening bracket", () => {
    ...
    expect(tokens[0][7].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
      "punctuation.definition.list.begin.python",
    ]);
    expect(tokens[0][8].value).toBe("j");
    expect(tokens[0][8].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
    ...
    expect(tokens[0][9].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
      "meta.structure.list.item.python",
      "meta.item-access.python",
    ...
    expect(tokens[0][10].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
      "meta.structure.list.item.python",
      "meta.item-access.python",
    ...
    expect(tokens[0][11].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
      "meta.structure.list.item.python",
      "meta.item-access.python",
    ...
    expect(tokens[0][12].scopes).toEqual([
      "source.python",
      "meta.structure.list.python",
  ...
  describe("f-strings", () => {
    it("tokenizes them", () => {
      ...
      expect(tokens[2]).toEqual({
        value: "hello",
        scopes: ["source.python", "string.quoted.single.single-line.format.python"],
      });
    ...
    it("tokenizes {{ and }} as escape characters", () => {
      ...
      expect(tokens[3]).toEqual({
        ...
        scopes: [
          "source.python",
          "string.quoted.single.single-line.format.python",
          "constant.character.escape.curly-bracket.python",
        ],
      });
    ...
    it("tokenizes unmatched closing curly brackets as invalid", () => {
      ...
      expect(tokens[3]).toEqual({
        ...
        scopes: [
          "source.python",
          "string.quoted.single.single-line.format.python",
          "invalid.illegal.closing-curly-bracket.python",
        ],
      });
      expect(tokens[4]).toEqual({
        value: "llo",
      ...
      it("tokenizes variables", () => {
        ...
        expect(tokens[2]).toEqual({
          ...
          scopes: [
            ...
            "punctuation.definition.interpolation.begin.bracket.curly.python",
          ],
        });
        expect(tokens[3]).toEqual({
          value: "abc",
        ...
        expect(tokens[4]).toEqual({
          ...
          scopes: [
            ...
            "string.quoted.single.single-line.format.python",
            "meta.interpolation.python",
            "punctuation.definition.interpolation.end.bracket.curly.python",
          ],
        });
      });
      ...
      it("tokenizes function and method calls", () => {
        ...
        expect(tokens[4]).toEqual({
          ...
          scopes: [
            ...
            "punctuation.separator.method.period.python",
          ],
        });
        expect(tokens[5]).toEqual({
          value: "decode",
        ...
        expect(tokens[7]).toEqual({
          ...
          scopes: [
            ...
            "meta.embedded.python",
            "meta.method-call.python",
            "meta.method-call.arguments.python",
            "string.quoted.double.single-line.python",
            "punctuation.definition.string.begin.python",
          ],
        });
      ...
      it("tokenizes backslashes as invalid", () => {
        ...
        expect(tokens[4]).toEqual({
          ...
          scopes: [
            ...
            "meta.interpolation.python",
            "meta.embedded.python",
            "invalid.illegal.backslash.python",
          ],
        });
        expect(tokens[6]).toEqual({
          value: "}",
  ...
  describe("binary strings", () => {
    it("tokenizes them", () => {
      ...
      expect(tokens[0]).toEqual({
        ...
        scopes: [
          "source.python",
          "string.quoted.single.single-line.binary.python",
          "storage.type.string.python",
        ],
      });
      expect(tokens[1]).toEqual({
      ...
      expect(tokens[2]).toEqual({
        value: "test",
        scopes: ["source.python", "string.quoted.single.single-line.binary.python"],
      });
      expect(tokens[3]).toEqual({
        value: "'",
        scopes: [
    ...
    it("tokenizes invalid characters", () => {
      ...
      expect(tokens[3]).toEqual({
        ...
        scopes: [
          "source.python",
          "string.quoted.single.single-line.binary.python",
          "invalid.illegal.character-out-of-range.python",
        ],
      });
      expect(tokens[4]).toEqual({
        value: "st",
...
`);
      ...
      expect(lines[1][0]).toEqual({
        value: '  Bla bla bla "wow" what\'s this?',
        scopes: ["source.python", "string.quoted.double.block.python"],
      });
...
`);
      ...
      expect(lines[1][0]).toEqual({
        value: '  Bla bla bla "wow" what\'s this?',
        scopes: ["source.python", "string.quoted.single.block.python"],
      });
      expect(lines[2][0]).toEqual({
        value: "'''",
        scopes: [
  ...
  it("tokenizes properties of self as self-type variables", () => {
    ...
    expect(tokens[0][0]).toEqual({
      value: "self",
      scopes: ["source.python", "variable.language.self.python"],
    });
    expect(tokens[0][1]).toEqual({
      value: ".",
      scopes: ["source.python", "punctuation.separator.property.period.python"],
    });
    expect(tokens[0][2]).toEqual({
      value: "foo",
      scopes: ["source.python", "variable.other.property.python"],
    });
  });
  ...
  it("tokenizes properties of a variable as variables", () => {
    ...
    expect(tokens[0][0]).toEqual({
      value: "bar",
      scopes: ["source.python", "variable.other.object.python"],
    });
    expect(tokens[0][1]).toEqual({
      value: ".",
      scopes: ["source.python", "punctuation.separator.property.period.python"],
...
FROM foo`,
          ...
          expect(tokens[1][0]).toEqual({
            value: "SELECT",
            scopes: ["source.python", scope, "meta.embedded.sql", "keyword.other.DML.sql"],
          });
          expect(tokens[1][1]).toEqual({
            value: " bar",
            scopes: ["source.python", scope, "meta.embedded.sql"],
          });
...
`);
          ...
          expect(tokens[1][2]).toEqual({
            value: "AS",
            scopes: ["source.python", scope, "meta.embedded.sql", "keyword.other.alias.sql"],
          });
    ...
    it("tokenizes SQL inline highlighting on single line with a CTE", () => {
      const { tokens } = grammar.tokenizeLine(
        "'WITH example_cte AS (SELECT bar FROM foo) SELECT COUNT(*) FROM example_cte'",
      );
      ...
      expect(tokens[0]).toEqual({
        ...
        scopes: [
          "source.python",
          "string.quoted.single.single-line.sql.python",
          "punctuation.definition.string.begin.python",
        ],
      });
      expect(tokens[1]).toEqual({
    ...
    it("tokenizes Python escape characters and formatting specifiers in SQL strings", () => {
      const { tokens } = grammar.tokenizeLine(
        "\"INSERT INTO url (image_uri) VALUES (\\'%s\\');\" % values",
      );
      ...
      expect(tokens[17]).toEqual({
        value: "%",
        scopes: ["source.python", "keyword.operator.arithmetic.python"],
      });
    });
