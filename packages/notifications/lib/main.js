...
const Notifications = {
  ...
  activate(_state) {
    ...
    this.subscriptions.add(
      atom.onWillThrowError(function ({ message, url, line, originalError, preventDefault }) {
        let match;
        if (originalError.name === "BufferedProcessError") {
          ...
          !/\/atom/i.test(message) &&
          (match = /spawn (.+) ENOENT/.exec(message))
        ) {
          message = `\
...
`;
    ...
    this.subscriptions.add(
      atom.config.observe("notifications.defaultTimeout", (value) => {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", "notifications:toggle-log", () =>
        atom.workspace.toggle(NotificationsLog.prototype.getURI()),
      ),
    );
    return this.subscriptions.add(
      atom.commands.add("atom-workspace", "notifications:clear-log", function () {
        for (notification of Array.from(atom.notifications.getNotifications())) {
