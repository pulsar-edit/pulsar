...
describe "Notifications", ->
  ...
  describe "when there are notifications before activation", ->
    beforeEach ->
      waitsForPromise ->
        # Wrapped in Promise.resolve so this test continues to work on earlier versions of Lumine
    ...
    describe "when an exception is thrown", ->
      ...
      describe "when the exception has no core or package paths in the stack trace", ->
        it "does not display a notification", ->
          ...
          # of Lumine or an Lumine package
      ...
      describe "when an exception is thrown from a package", ->
        beforeEach ->
          ...
          spyOn(atom, 'inDevMode').andReturn false
          generateFakeFetchResponses()
          spyOn(UserUtils, 'getPackageVersionShippedWithLumine').andCallFake -> '0.0.0'
          generateException()
          notificationContainer = workspaceElement.querySelector('atom-notifications')
          fatalError = notificationContainer.querySelector('atom-notification.fatal')
      ...
      describe "when the package is out of date", ->
        ...
        describe "when the package is a core package", ->
          ...
          describe "when the locally installed version is lower than Lumine's version", ->
            beforeEach ->
              versionShippedWithLumine = '0.10.0'
              UserUtilities = require '../lib/user-utilities'
              spyOn(UserUtilities, 'getPackageVersionShippedWithLumine').andCallFake -> versionShippedWithLumine
          ...
          describe "when the locally installed version matches Lumine's version", ->
            beforeEach ->
      ...
      # TODO: Re-enable when Lumine have a way to check this
      # describe "when Lumine is out of date", ->
      #   beforeEach ->
      ...
      #     expect(button).not.toExist()
      #
      #   it "tells the user that Lumine is out of date", ->
      #     fatalNotification = fatalError.querySelector('.fatal-notification')
      #     expect(fatalNotification.textContent).toContain 'Lumine is out of date'
      #
