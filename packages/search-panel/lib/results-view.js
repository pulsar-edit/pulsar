...
module.exports = class ResultsView {
  constructor({ model, collapseNewResults = false }) {
    this.model = model;
    this.collapseNewResults = collapseNewResults;
    this.pixelOverdraw = 100;

    this.resultRowGroups = Object.values(model.results).map(
      (result) => new ResultRowGroup(result, this.model.getFindOptions()),
    );
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.commands.add(this.element, {
        ...
        "search-panel:copy-path": this.copyPath.bind(this),
        "search-panel:open-in-new-tab": this.openInNewTab.bind(this),
      }),
    );
  }
  ...
  setCollapseNewResults(value) {
    this.collapseNewResults = value;
  }
  ...
  async invalidateItemHeights() {
    ...
    if (
      matchRowHeight !== this.matchRowHeight ||
  ...
  invalidateRowGroup(firstRowIndex, groupIndex) {
    ...
    this.spliceRows(firstRowIndex, this.rowGroupLengths[groupIndex], rowGroup.displayedRows());
    this.rowGroupLengths[groupIndex] = rowGroup.displayedRows().length;
  }

  findGroupIndex(filePath) {
    return this.resultRowGroups.findIndex((g) => g.result.filePath === filePath);
  }
  ...
  isAtBottom() {
    if (!this.refs.listView) return true;
    const el = this.refs.listView.element;
    return el.scrollTop + el.clientHeight >= el.scrollHeight - 1;
  }

  // These four methods are the only ones allowed to modify this.resultRowGroups
  async didAddResult({ result }) {
    const atBottom = this.isAtBottom();

    const rowGroup = new ResultRowGroup(result, this.model.getFindOptions());

    // Respect the collapse state for new results
    if (this.collapseNewResults) {
      rowGroup.data.isCollapsed = true;
    }

    const groupIndex = this.resultRowGroups.length;
    this.resultRowGroups.push(rowGroup);
    this.rowGroupLengths.push(rowGroup.displayedRows().length);
    ...
    if (this.selectedRowIndex !== -1 && atBottom) {
      this.scrollToBottom();
    }
  }

  didSetResult({ result, filePath }) {
    const groupIndex = this.findGroupIndex(filePath);
    const rowGroup = this.resultRowGroups[groupIndex];
    const rowIndex = this.getRowCountBefore(groupIndex);
  ...
  handleClick(event) {
    ...
    const groupIndex = this.findGroupIndex(clickedItem.dataset.filePath);
    ...
    if (clickedItem.matches(".context-row, .match-row")) {
      ...
      const rowIndex = binarySearch(
        group.rows,
        clickedItem.dataset.matchLineNumber,
  ...
  async confirmResult({ pending } = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await atom.workspace.open(selectedRow.group.result.filePath, {
        ...
        searchAllPanes: true,
        split:
          reverseDirections[atom.config.get("search-panel.projectSearchResultsPaneSplitDirection")],
      });
  ...
  async openInNewTab() {
    if (this.selectedRowIndex !== -1) {
      ...
      if (result) {
        ...
        const exists = editors.some((editor) => editor.getPath() == filepath);
  ...
  sortByFile() {
    this.resultRowGroups.sort((a, b) => filepathComp(a.result.filePath, b.result.filePath));
    this.resultRows = [];
    this.rowGroupLengths = [];
    for (const group of this.resultRowGroups) {
      this.rowGroupLengths.push(group.displayedRows().length);
      this.resultRows.push(...group.displayedRows());
    }
    if (this.selectedRowIndex === -1 && this.resultRows.length > 0) {
      this.selectedRowIndex = 0;
    }
    this.scrollToSelectedMatch();
    return etch.update(this);
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = atom.config.get(
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = atom.config.get(
  ...
  incrementTrailingContextLines() {
    const searchContextLineCountAfter = atom.config.get("search-panel.searchContextLineCountAfter");
    if (this.model.getFindOptions().trailingContextLineCount < searchContextLineCountAfter) {
      this.model.getFindOptions().trailingContextLineCount++;
      return this.contextLinesChanged();
    }
