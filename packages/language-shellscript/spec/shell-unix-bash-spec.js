...
describe("Shell script grammar", function () {
  ...
  it("tokenizes strings inside variable constructs", function () {
    ...
    expect(tokens[2]).toEqual({
      value: "root",
      scopes: ["source.shell", "variable.other.bracket.shell", "string.quoted.single.shell"],
    });
  ...
  it("tokenizes if correctly when it's a parameter", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "dd if=/dev/random of=/dev/null",
      scopes: ["source.shell"],
    });
  });
    ...
    return (() => {
      ...
      for (let openingBracket in brackets) {
        ...
        const { tokens } = grammar.tokenizeLine(
          "if " + openingBracket + " -f /var/log/messages " + closingBracket,
        );
        expect(tokens[0]).toEqual({
          value: "if",
          scopes: ["source.shell", "meta.scope.if-block.shell", "keyword.control.shell"],
        });
        expect(tokens[2]).toEqual({
  ...
  it("tokenizes for...in loops", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "for",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell", "keyword.control.shell"],
    });
    expect(tokens[2]).toEqual({
      value: "variable",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell", "variable.other.loop.shell"],
    });
    expect(tokens[4]).toEqual({
      value: "in",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell", "keyword.control.shell"],
    ...
    expect(tokens[5]).toEqual({
      value: " file ",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell"],
    });
    expect(tokens[6]).toEqual({
      value: "do",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell", "keyword.control.shell"],
    ...
    expect(tokens[9]).toEqual({
      value: " in",
      scopes: ["source.shell", "meta.scope.for-in-loop.shell", "comment.line.number-sign.shell"],
    });
  });
  ...
  it("doesn't tokenize keywords when they're part of a phrase", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "this/function",
      scopes: ["source.shell", "meta.function.shell", "entity.name.function.shell"],
    });
    expect(tokens[2]).toEqual({
      value: "()",
      scopes: ["source.shell", "meta.function.shell", "punctuation.definition.arguments.shell"],
    });
...
`);
      ...
      expect(tokens[0][1]).toEqual({
        value: "cmd",
        scopes: ["source.shell", "variable.other.normal.shell"],
      });
      expect(tokens[0][3]).toEqual({
        value: "<<<",
        scopes: ["source.shell", "meta.herestring.shell", "keyword.operator.herestring.shell"],
      });
      ...
      expect(tokens[1][0]).toEqual({
        value: "lorem ipsum",
        scopes: ["source.shell", "meta.herestring.shell", scope],
      });
...
`);
      ...
      expect(tokens[0][4]).toEqual({
        value: " ",
        scopes: ["source.shell", "meta.herestring.shell"],
      });
    ...
    expect(tokens[5]).toEqual({
      value: "TEST",
      scopes: ["source.shell", "meta.herestring.shell", "string.unquoted.herestring.shell"],
    });
...
`);
      expect(tokens[0][0]).toEqual({
        ...
        scopes: [
          "source.shell",
          "string.unquoted.heredoc." + scope + ".shell",
...
`);
      ...
      expect(tokens[0][1]).toEqual({
        value: " ",
        scopes: ["source.shell", "string.unquoted.heredoc." + scope + ".shell"],
      });
...
`);
      expect(tokens[0][0]).toEqual({
        ...
        scopes: [
          "source.shell",
          "string.unquoted.heredoc.no-indent." + scope + ".shell",
          "keyword.operator.heredoc.shell",
        ],
      });
      expect(tokens[0][2]).toEqual({
...
`);
      ...
      expect(tokens[0][1]).toEqual({
        value: "- ",
        scopes: ["source.shell", "string.unquoted.heredoc.no-indent." + scope + ".shell"],
      });
      expect(tokens[0][2]).toEqual({
        value: delim,
        scopes: [
    ...
    const delims = ["RANDOMTHING", "RUBY@1.8", "END-INPUT"];
...
`);
      ...
      expect(tokens[1][0]).toEqual({
        value: "stuff",
        scopes: ["source.shell", "string.unquoted.heredoc.expanded.shell"],
      });
      expect(tokens[2][0]).toEqual({
        value: delim,
        scopes: [
...
`);
        ...
        expect(tokens[1][0]).toEqual({
          value: "stuff",
          scopes: ["source.shell", "string.unquoted.heredoc.shell"],
        });
        result.push(
          expect(tokens[2][0]).toEqual({
            value: delim,
  ...
  it("tokenizes shebangs", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "/bin/sh",
      scopes: ["source.shell", "comment.line.number-sign.shebang.shell"],
    });
  });
  ...
  it("tokenizes comments", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "comment",
      scopes: ["source.shell", "comment.line.number-sign.shell"],
    });
  });
  ...
  it("tokenizes nested variable expansions", function () {
    ...
    expect(tokens[2]).toEqual({
      value: "C",
      scopes: ["source.shell", "variable.other.bracket.shell", "variable.other.bracket.shell"],
    });
    expect(tokens[3]).toEqual({
      value: "}",
      scopes: [
  ...
  it("tokenizes case blocks", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "case",
      scopes: ["source.shell", "meta.scope.case-block.shell", "keyword.control.shell"],
    });
    expect(tokens[1]).toEqual({
      value: " word ",
      scopes: ["source.shell", "meta.scope.case-block.shell"],
    });
    expect(tokens[2]).toEqual({
      value: "in",
      scopes: [
    ...
    expect(tokens[3]).toEqual({
      value: " ",
      scopes: ["source.shell", "meta.scope.case-block.shell", "meta.scope.case-body.shell"],
    });
    expect(tokens[4]).toEqual({
      value: "esac",
      scopes: [
  ...
  it("does not confuse strings and functions", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "echo",
      scopes: ["source.shell", "support.function.builtin.shell"],
    });
    expect(tokens[2]).toEqual({
      value: '"',
      scopes: [
    ...
    expect(tokens[3]).toEqual({
      value: "()",
      scopes: ["source.shell", "string.quoted.double.shell"],
    });
    expect(tokens[4]).toEqual({
      value: '"',
      scopes: [
