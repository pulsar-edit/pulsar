    ...
    (() => {
      ...
      for (let delim of prompts) {
        ...
        expect(tokens[0]).toEqual({
          value: delim,
          scopes: ["text.shell-session", "punctuation.separator.prompt.shell-session"],
        });
        expect(tokens[1]).toEqual({ value: " ", scopes: ["text.shell-session"] });
        result.push(
          expect(tokens[2]).toEqual({
            value: "echo",
            scopes: ["text.shell-session", "source.shell", "support.function.builtin.shell"],
          }),
        );
      }
      return result;
...
`);
        ...
        expect(lines[0][3]).toEqual({
          value: ` ${sigil}μμ`,
          scopes: ["text.shell-session", "source.shell"],
        });
        result.push(
          expect(lines[1][0]).toEqual({
            value: `O${sigil}tput Ω`,
            scopes: ["text.shell-session", "meta.output.shell-session"],
          }),
        );
      }
      return result;
  ...
  it("tokenizes prompts with prefixes", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "user@machine",
      scopes: ["text.shell-session", "entity.other.prompt-prefix.shell-session"],
    });
    expect(tokens[1]).toEqual({ value: " ", scopes: ["text.shell-session"] });
    expect(tokens[2]).toEqual({
      value: "$",
