describe("TextMate Gemfile grammar", () => {
  ...
  it("tokenizes double-quoted strings", () => {
    ...
    expect(tokens[1]).toEqual({
      value: "foo",
      scopes: ["source.ruby.gemfile", "string.quoted.double.interpolated.ruby"],
    });
  ...
  it("tokenizes single-quoted strings", () => {
    ...
    expect(tokens[1]).toEqual({
      value: "foo",
      scopes: ["source.ruby.gemfile", "string.quoted.single.ruby"],
    });
    expect(tokens[2]).toEqual({
      value: "'",
      scopes: [
  ...
  it("tokenizes group names", () => {
    ...
    expect(tokens[1]).toEqual({
      value: "foo",
      scopes: ["source.ruby.gemfile", "constant.other.symbol.ruby"],
    });
  });
  ...
  it("tokenizes group properly in ruby code", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "do",
      scopes: ["source.ruby.gemfile", "keyword.control.start-block.ruby"],
    });
    expect(tokens[1]).toEqual({ value: " ", scopes: ["source.ruby.gemfile"] });
    expect(tokens[2]).toEqual({
      value: "|",
      scopes: ["source.ruby.gemfile", "punctuation.separator.variable.ruby"],
    });
    expect(tokens[3]).toEqual({
      value: "group",
      scopes: ["source.ruby.gemfile", "variable.other.block.ruby"],
    });
