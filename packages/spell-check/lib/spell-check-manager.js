...
class SpellCheckerManager {
  ...
  setGlobalArgs(data) {
    ...
    //
    // We have three basic checkers that are bundled:
    //
    ...
    // change the system checker because we show different messages if we
    // cannot find a locale based on the use of the system checker.
  ...
  removeSpellChecker(spellChecker) {
    let index = this.checkers.findIndex((c) => c === spellChecker);
    if (index === -1) return false;
    this.checkers.splice(index, 1);
    return true;
  }

  async check(args, text) {
    ...
    for (let checker of this.checkers) {
      ...
      // Get the possibly asynchronous results which include positive (correct)
      // and negative (incorrect) ranges. If we have an incorrect range but no
      // correct, everything not in incorrect is considered correct.
    ...
    try {
      ...
      for (let results of allResults) {
        if (results.invertIncorrectAsCorrect && results.incorrect) {
          // We need to add the opposite of the incorrect as correct elements
          // in the list. We do this by creating a subtraction.
          const invertedCorrect = new multirange.MultiRange([[0, text.length]]);
          const removeRange = new multirange.MultiRange([]);
          for (let range of results.incorrect) {
          ...
          for (let range of results.correct) {
      ...
      // Build up an intersection of all the incorrect ranges. We only treat a
      // word as being incorrect if *every* checker that provides negative
      // values treats it as incorrect.
      //
      // We know there is at least one item in this list, so pull that out. If
      // that is the only one, we don't have to do any additional work.
      // Otherwise we compare every other one against it, removing any elements
      // that aren't an intersection — which, hopefully, will produce a smaller
      ...
      // If we have no intersection, then we have no problems to report.
      ...
      // Convert the text ranges (index into the string) into Pulsar buffer
      ...
      while (
        ...
        while (true) {
          let range = intersection.ranges[rangeIndex];
          if (range && range[0] < lineEndIndex) {
            // Figure out the character range of this line. We need this
            // because `addMisspellings` doesn't handle jumping across lines
            // easily and the use of the number ranges is inclusive.
            const lineRange = new multirange.MultiRange([]).appendRange(
              lineBeginIndex,
              lineEndIndex
            );
            const rangeRange = new multirange.MultiRange([]).appendRange(
              range[0],
              range[1]
            );
            lineRange.intersect(rangeRange);

            // The range we have here includes whitespace between two
            // concurrent tokens ("zz zz zz" shows up as a single misspelling).
            // The original version would split the example into three separate
            // ones, so we do the same thing, but only for the ranges within
            // the line.
            ...
            // If this line is beyond the limits of our current range, we move
            // to the next one; otherwise we loop again to reuse this range
      ...
      console.error(`Error during spellcheck:`);
      console.error(err);
    }
  }
  ...
  suggest(args, word) {
    ...
    for (let checker of this.checkers) {
      ...
      let index = 0;
      let priority = checker.getPriority();

      for (let suggestion of checker.suggest(args, word)) {
        suggestions.push({
          isSuggestion: true,
          priority,
          index: index++,
  ...
  addMisspellings(misspellings, row, range, lineBeginIndex, text) {
    // Get the substring of text. If there is no space, we can return the
    // entire result.
    const substring = text.substring(range[0], range[1]);

    if (/\s+/.test(substring)) {
      // We have a space, break it into individual components and add each one
      // to the misspelling list.
  ...
  init() {
    ...
    if (this.useLocales && this.localeCheckers === null) {
      ...
      // choice, but it is a reasonable default — plus it can be used with the
      // fallback path of the `spellchecker` package.
      if (!this.locales.length) {
        inferredLocale = true;
        this.locales = ['en_US'];
      }
