describe("SQL grammar", () => {
  ...
  it("quotes strings", () => {
    ...
    expect(tokens[0]).toEqual({
      value: '"',
      scopes: ["source.sql", "string.quoted.double.sql", "punctuation.definition.string.begin.sql"],
    });
    ...
    expect(tokens[2]).toEqual({
      value: '"',
      scopes: ["source.sql", "string.quoted.double.sql", "punctuation.definition.string.end.sql"],
    });
  });
...
`);
    ...
    expect(lines[1][1]).toEqual({
      value: "(",
      scopes: ["source.sql", "punctuation.definition.parameters.bracket.round.begin.sql"],
    });
    expect(lines[1][2]).toEqual({
      value: ")",
      scopes: ["source.sql", "punctuation.definition.parameters.bracket.round.end.sql"],
    });
...
`);
  ...
  it("tokenizes comments", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "--",
      scopes: ["source.sql", "comment.line.double-dash.sql", "punctuation.definition.comment.sql"],
    });
    expect(tokens[1]).toEqual({
      value: " comment",
    ...
    expect(tokens[0]).toEqual({
      value: "/*",
      scopes: ["source.sql", "comment.block.sql", "punctuation.definition.comment.sql"],
    });
  ...
  describe("punctuation", () => {
    it("tokenizes parentheses", () => {
      ...
      expect(tokens[4]).toEqual({
        value: "(",
        scopes: ["source.sql", "punctuation.definition.section.bracket.round.begin.sql"],
      });
      ...
      expect(tokens[10]).toEqual({
        value: ")",
        scopes: ["source.sql", "punctuation.definition.section.bracket.round.end.sql"],
      });
    ...
    it("tokenizes semicolons", () => {
      ...
      expect(tokens[2]).toEqual({
        value: ";",
        scopes: ["source.sql", "punctuation.terminator.statement.semicolon.sql"],
      });
    });
  });
});
