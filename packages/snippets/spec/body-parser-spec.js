...
describe("Snippet Body Parser", () => {
  it("parses a snippet with no special behavior", () => {
    ...
    expect(bodyTree).toEqual(["${} $ n $}1} ${/upcase/} \n world ${||}"]);
  });
  ...
  describe("for snippets with variables", () => {
    ...
    it("parses variables with placeholders", () => {
      ...
      expectMatch("${f:foo$1 $VAR}", [
        {
          variable: "f",
          content: ["foo", { index: 1, content: [] }, " ", { variable: "VAR" }],
        },
      ]);

      // Allows a colon as part of the placeholder value.
      expectMatch("${TM_SELECTED_TEXT:foo:bar}", [
        {
  ...
  describe("for snippets with tabstops", () => {
    ...
    it("parses if-else syntax", () => {
      ...
      // else with `:` syntax
      expectMatch("$1 ${1/(?:(wat)|^.*$)$/${1:fallback}/}", [
        { index: 1, content: [] },
        " ",
        {
          index: 1,
        ...
        " ",
        {
          ...
          substitution: {
            ...
            replace: [
              {
                backreference: 1,
                iftext: [{ escape: "u" }, { backreference: 1 }],
                elsetext: "",
              },
            ],
          },
                  ...
                  replace: [
                    {
                      backreference: 2,
                      iftext: "",
                      elsetext: [{ escape: "u" }, { backreference: 1 }],
                    },
                  ],
                },
              },
    ...
    it("recognizes escape characters in if/else syntax", () => {
      expectMatch("$1 ${1/(?:(wat)|^.*$)$/${1:?hey\\:hey:nah}/}", [
        { index: 1, content: [] },
        " ",
        {
          index: 1,
      ...
      expectMatch("$1 ${1/(?:(wat)|^.*$)$/${1:?hey:n\\}ah}/}", [
        { index: 1, content: [] },
        " ",
        {
          index: 1,
    ...
    it("parses nested tabstops", () => {
      expectMatch("${1:place${2:hol${3:der}}}", [
        {
          index: 1,
          content: ["place", { index: 2, content: ["hol", { index: 3, content: ["der"] }] }],
        },
      ]);
