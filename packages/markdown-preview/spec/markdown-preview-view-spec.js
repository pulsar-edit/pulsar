...
describe("MarkdownPreviewView", function () {
  ...
  describe("in-page anchor links", function () {
    it("uses GitHub-compatible heading ids with the original parser", async function () {
      ...
      const launchTarget = preview.element.querySelector("#user-content--launch");
      const titleTarget = preview.element.querySelector("#user-content-title");
      const htmlTarget = preview.element.querySelector("#user-content-hello-world");
      const entityLink = preview.element.querySelector('a[href="#caf%C3%A9"]');
      ...
      launchLink.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
      entityLink.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
    ...
    it("prefers the generated prefixed id over a colliding raw id", function () {
      ...
      const anchor = preview.element.querySelector('a[href="#user-content-foo"]');
      const rawTarget = preview.element.querySelector("#user-content-foo");
      const prefixedTarget = preview.element.querySelector("#user-content-user-content-foo");
...
`);
  ...
  describe("yaml front matter", function () {
    it("creates a table with the YAML variables", async () => {
      ...
      expect([...preview.element.querySelectorAll("table th")].map((el) => el.textContent)).toEqual(
        ["variable1", "array", "object", "key"],
      );
      expect([...preview.element.querySelectorAll("table td")].map((el) => el.textContent)).toEqual(
        ["value1", "foo,bar", "keyvalue2", "value2"],
      );
    });
  });
  ...
  describe("text selections", function () {
    it("adds the `has-selection` class to the preview depending on if there is a text selection", async () => {
      ...
      await conditionPromise(() => preview.element.classList.contains("has-selection") === false);
    });
  });
  ...
  describe("when core:save-as is triggered", function () {
    ...
    it("saves the rendered HTML and opens it", async () => {
      ...
      expect(
        element.querySelector(".line .syntax--source.syntax--js .syntax--constant.syntax--numeric")
          .innerText,
      ).toBe("3");
      expect(
        element.querySelector(".line .syntax--source.syntax--js .syntax--keyword.syntax--control")
          .innerText,
      ).toBe("if");
      expect(
        element.querySelector(".line .syntax--source.syntax--js .syntax--constant.syntax--numeric")
...
`);
  ...
  describe("when GitHub styles are enabled", () => {
    ...
    it("uses the GitHub styles", async () => {
      ...
      expect([expectedColors.light, expectedColors.dark]).toContain(
