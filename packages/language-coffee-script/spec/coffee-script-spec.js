...
describe("CoffeeScript grammar", function () {
  ...
  it("tokenizes classes", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "class_",
      scopes: ["source.coffee", "meta.function-call.coffee", "entity.name.function.coffee"],
    });
    ...
    expect(tokens[0]).toEqual({
      value: "[",
      scopes: ["source.coffee", "punctuation.definition.array.begin.bracket.square.coffee"],
    });
    ...
    expect(tokens[4]).toEqual({
      value: "]",
      scopes: ["source.coffee", "punctuation.definition.array.end.bracket.square.coffee"],
    });
...
`);
    expect(lines[0][0]).toEqual({
      value: "###",
      scopes: ["source.coffee", "comment.block.coffee", "punctuation.definition.comment.coffee"],
    });
    ...
    expect(tokens[19]).toEqual({
      value: "->",
      scopes: ["source.coffee", "meta.function.inline.coffee", "storage.type.function.coffee"],
    });
  });
...
`);
  ...
  it("tokenizes variable assignments", function () {
    ...
    expect(tokens[2]).toEqual({
      value: "&=",
      scopes: ["source.coffee", "keyword.operator.assignment.compound.bitwise.coffee"],
    });
    expect(tokens[3]).toEqual({ value: " b", scopes: ["source.coffee"] });
  ...
  it("tokenizes compound operators properly", function () {
    let tokens;
    const assignmentOperators = ["and=", "or=", "&&=", "||=", "/=", "*=", "%=", "+=", "-="];
    const bitwiseOperators = ["<<=", ">>=", ">>>=", "&=", "|=", "^="];
    const comparisonOperators = ["==", "!=", "<=", ">="];

    for (let assignmentOperator of Array.from(assignmentOperators)) {
    ...
    for (let bitwiseOperator of Array.from(bitwiseOperators)) {
    ...
    return (() => {
      const result = [];
      for (let comparisonOperator of Array.from(comparisonOperators)) {
  ...
  it("tokenizes operators properly", function () {
    ...
    for (let logicalOperator of Array.from(logicalOperators)) {
    ...
    for (let decrementOperator of Array.from(decrementOperators)) {
    ...
    for (let incrementOperator of Array.from(incrementOperators)) {
    ...
    for (let splatOperator of Array.from(splatOperators)) {
    ...
    for (let existentialOperator of Array.from(existentialOperators)) {
  ...
  it("does not tokenize non-operators as operators", function () {
    ...
    return (() => {
      const result = [];
      for (let notOperator of Array.from(notOperators)) {
        const { tokens } = grammar.tokenizeLine(notOperator);
        result.push(
  ...
  describe("properties", function () {
    it("tokenizes properties", function () {
      ...
      expect(tokens[1]).toEqual({
        value: ".",
        scopes: ["source.coffee", "punctuation.separator.property.period.coffee"],
      });
      expect(tokens[2]).toEqual({
        value: "property",
    ...
    it("tokenizes objects, methods, and properties using :: prototype syntax", function () {
      ...
      expect(tokens[1]).toEqual({
        value: "::",
        scopes: ["source.coffee", "meta.method-call.coffee", "keyword.operator.prototype.coffee"],
      });
      expect(tokens[2]).toEqual({
        value: "toString",
        scopes: ["source.coffee", "meta.method-call.coffee", "support.function.coffee"],
      });
    });
  });
...
`);
  ...
  describe("function calls", function () {
    it("tokenizes function calls", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "arg1",
        scopes: ["source.coffee", "meta.function-call.coffee", "meta.arguments.coffee"],
      });
      expect(tokens[3]).toEqual({
        value: ",",
        scopes: [
    ...
    it("tokenizes illegal function calls", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "0illegal",
        scopes: ["source.coffee", "meta.function-call.coffee", "invalid.illegal.identifier.coffee"],
      });
      expect(tokens[1]).toEqual({
        value: "(",
        scopes: [
  ...
  describe("functions", function () {
    it("tokenizes regular functions", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "=",
        scopes: ["source.coffee", "meta.function.coffee", "keyword.operator.assignment.coffee"],
      });
      expect(tokens[3]).toEqual({ value: " ", scopes: ["source.coffee", "meta.function.coffee"] });
      expect(tokens[4]).toEqual({
        value: "->",
        scopes: ["source.coffee", "meta.function.coffee", "storage.type.function.coffee"],
      });
      expect(tokens[5]).toEqual({ value: " ", scopes: ["source.coffee"] });
      expect(tokens[6]).toEqual({
        value: "1",
      ...
      expect(tokens[6]).toEqual({
        value: " ",
        scopes: ["source.coffee", "meta.function.coffee", "meta.parameters.coffee"],
      });
      expect(tokens[7]).toEqual({
        value: "b",
        scopes: [
    ...
    it("tokenizes inline functions", function () {
      ...
      expect(tokens[3]).toEqual({
        value: " ",
        scopes: ["source.coffee", "meta.function.inline.coffee", "meta.parameters.coffee"],
      });
      expect(tokens[4]).toEqual({
        value: "arg2",
        scopes: [
  ...
  describe("method calls", () =>
    it("tokenizes method calls", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "b",
        scopes: ["source.coffee", "meta.method-call.coffee", "entity.name.function.coffee"],
      });
      expect(tokens[3]).toEqual({
        value: "(",
        scopes: [
      ...
      expect(tokens[4]).toEqual({
        value: "c",
        scopes: ["source.coffee", "meta.method-call.coffee", "meta.arguments.coffee"],
      });
  ...
  describe("destructuring assignments", function () {
    it("tokenizes object and array destructuring", function () {
      ...
      expect(tokens[3]).toEqual({
        value: " ",
        scopes: ["source.coffee", "meta.variable.assignment.destructured.array.coffee"],
      });
      expect(tokens[4]).toEqual({
        value: "y",
        scopes: [
  ...
  describe("for loops", function () {
    ...
    it("tokenizes loops using arrays", function () {
      const { tokens } = grammar.tokenizeLine("for food in ['broccoli', 'spinach', 'chocolate']");
      expect(tokens[0]).toEqual({
        value: "for",
        scopes: ["source.coffee", "keyword.control.coffee"],
      });
    ...
    it("tokenizes loops using the optional `when` keyword", function () {
      const { tokens } = grammar.tokenizeLine("for food in foods when food isnt chocolate");
      expect(tokens[0]).toEqual({
        value: "for",
        scopes: ["source.coffee", "keyword.control.coffee"],
      });
  ...
  describe("regular expressions", function () {
    ...
    it("tokenises multi-line regular expressions", function () {
      ...
      expect(tokens[3]).toEqual({
        value: "XYZ",
        scopes: ["source.coffee", "string.regexp.multiline.coffee", "meta.group.regexp"],
      });
      expect(tokens[4]).toEqual({
        value: ")",
        scopes: [
...
`);
  ...
  describe("jsx", function () {
    it("tokenises HTML tags", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "<",
        scopes: ["source.coffee", "meta.tag.coffee", "punctuation.definition.tag.coffee"],
      });
      expect(tokens[1]).toEqual({
        value: "div",
    ...
    it("tokenises HTML tags with attributes", function () {
      const { tokens } = grammar.tokenizeLine("<div class='myclass' id=\"myid\">");
      expect(tokens[0]).toEqual({
        value: "<",
        scopes: ["source.coffee", "meta.tag.coffee", "punctuation.definition.tag.coffee"],
      });
      ...
      expect(tokens[3]).toEqual({
        value: "class",
        scopes: ["source.coffee", "meta.tag.coffee", "entity.other.attribute-name.coffee"],
      });
      expect(tokens[4]).toEqual({
        value: "=",
        scopes: ["source.coffee", "meta.tag.coffee", "keyword.operator.assignment.coffee"],
      });
      expect(tokens[5]).toEqual({
        value: "'",
        scopes: [
      ...
      expect(tokens[6]).toEqual({
        value: "myclass",
        scopes: ["source.coffee", "meta.tag.coffee", "string.quoted.single.coffee"],
      });
      expect(tokens[7]).toEqual({
        value: "'",
        scopes: [
      ...
      expect(tokens[12]).toEqual({
        value: "myid",
        scopes: ["source.coffee", "meta.tag.coffee", "string.quoted.double.coffee"],
      });
      expect(tokens[13]).toEqual({
        value: '"',
        scopes: [
