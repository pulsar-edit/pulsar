...
`
    ...
    const {tokens} = grammar.tokenizeLine("identity = ###::<T>### (value ###: T ###) ###: T ### ->");
...
`
  ...
  it("tokenizes compound operators properly", function() {
    ...
    return (() => {
      ...
      for (let comparisonOperator of Array.from(comparisonOperators)) {
        ...
        result.push(expect(tokens[0]).toEqual({value: comparisonOperator, scopes: ["source.coffee", "keyword.operator.comparison.coffee"]}));
      }
      return result;
    })();
});
  ...
  it("tokenizes operators properly", function() {
    ...
    return (() => {
      ...
      for (let keyword of Array.from(keywords)) {
        ...
        result.push(expect(tokens[0]).toEqual({value: keyword, scopes: ["source.coffee", `keyword.operator.${keyword}.coffee`]}));
      }
      return result;
    })();
});
  ...
  it("does not tokenize non-operators as operators", function() {
    ...
    return (() => {
      ...
      for (let notOperator of Array.from(notOperators)) {
        ...
        result.push(expect(tokens[0]).not.toEqual({value: notOperator, scopes: ["source.coffee", "keyword.operator.coffee"]}));
      }
      return result;
    })();
});
  ...
  it("verifies that regular expressions have explicit count modifiers", function() {
    let source = fs.readFileSync(path.resolve(__dirname, '..', 'grammars', 'coffeescript.cson'), 'utf8');
...
`
  ...
  describe("method calls", () => it("tokenizes method calls", function() {
  ...
  describe("destructuring assignments", function() {
    ...
    it("tokenizes nested destructuring assignments", function() {
      const {tokens} = grammar.tokenizeLine("{poet: {name, address: [street, city]}} = futurists");
      expect(tokens[0]).toEqual({value: "{", scopes: ["source.coffee", "meta.variable.assignment.destructured.object.coffee", "punctuation.definition.destructuring.begin.bracket.curly.coffee"]});
    ...
    it("tokenizes multiple nested destructuring assignments", function() {
      const {tokens} = grammar.tokenizeLine("{start: {row: startRow}, end: {row: endRow}} = range");
      expect(tokens[0]).toEqual({value: "{", scopes: ["source.coffee", "meta.variable.assignment.destructured.object.coffee", "punctuation.definition.destructuring.begin.bracket.curly.coffee"]});
      expect(tokens[4]).toEqual({value: "{", scopes: ["source.coffee", "meta.variable.assignment.destructured.object.coffee", "meta.variable.assignment.destructured.object.coffee", "punctuation.definition.destructuring.begin.bracket.curly.coffee"]});
