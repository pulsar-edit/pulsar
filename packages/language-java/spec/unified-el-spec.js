describe("Unified expression language grammar", function () {
  ...
  describe("operators", function () {
    it("tokenizes the ternary operator", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "?",
        scopes: ["source.java.el", "keyword.control.ternary.java.el"],
      });
    ...
    it("parses the comparison operator `==`", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "==",
        scopes: ["source.java.el", "keyword.operator.comparison.java.el"],
      });
    });
    ...
    it("parses the empty operators", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "empty",
        scopes: ["source.java.el", "keyword.operator.empty.java.el"],
      });
    });
    ...
    it("parses the arithmetic operator `-`", function () {
      ...
      expect(tokens[2]).toEqual({
        value: "-",
        scopes: ["source.java.el", "keyword.operator.arithmetic.java.el"],
      });
    });
    ...
    it("parses the logical operator `!`", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "!",
        scopes: ["source.java.el", "keyword.operator.logical.java.el"],
      });
    });
  ...
  describe("literals", function () {
    it("parses boolean literals", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "true",
        scopes: ["source.java.el", "constant.boolean.java.el"],
      });
    ...
    it("parses numeric literals", function () {
      ...
      expect(tokens[0]).toEqual({
        value: "0",
        scopes: ["source.java.el", "constant.numeric.java.el"],
      });
    ...
    it("tokenizes single quoted string literals", function () {
      ...
      expect(tokens[0]).toEqual({
        ...
        scopes: [
          ...
          "punctuation.definition.string.begin.java.el",
        ],
      });
      expect(tokens[1]).toEqual({
        value: "foo",
        scopes: ["source.java.el", "string.quoted.single.java.el"],
      });
      expect(tokens[2]).toEqual({
        value: "\\n",
        scopes: [
          "source.java.el",
          "string.quoted.single.java.el",
          "constant.character.escape.java.el",
        ],
      });
      ...
      expect(tokens[6]).toEqual({
        ...
        scopes: [
          "source.java.el",
          "string.quoted.single.java.el",
          "punctuation.definition.string.end.java.el",
        ],
      });
    });
  ...
  it("tokenizes function calls", function () {
    ...
    expect(tokens[1]).toEqual({
      value: ":",
      scopes: ["source.java.el", "namespace.java.el", "punctuation.separator.namespace.java.el"],
    });
    expect(tokens[2]).toEqual({ value: "split", scopes: ["source.java.el"] });
    expect(tokens[3]).toEqual({
      value: "(",
      scopes: ["source.java.el", "meta.brace.round.java.el"],
    });
  ...
  it("tokenizes a computed property access", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "[",
      scopes: ["source.java.el", "meta.brace.square.java.el"],
    });
    expect(tokens[2]).toEqual({
      value: "0",
      scopes: ["source.java.el", "constant.numeric.java.el"],
