...
describe("FuzzyFinder", () => {
  ...
  function waitForInitialCrawlerToFinish(fuzzyFinder) {
    return new Promise((resolve) =>
      fuzzyFinder.loadPathsTask.on("task:completed", () => resolve()),
    );
  }
  ...
  function parseResults(elementContainer) {
    return Array.from(elementContainer.querySelectorAll("li")).map((element) => ({
    ...
    describe(`file-finder behavior (ripgrep=${useRipGrep})`, () => {
      ...
      describe("toggling", () => {
        describe("when the project has multiple paths", () => {
          it("shows or hides the fuzzy-finder and returns focus to the active editor if it is already showing", async () => {
            ...
            projectView.selectListView.refs.queryEditor.insertText(
              "this should not show up next time we toggle",
            );
          ...
          it("shows all files for the current project and selects the first", async () => {
            ...
            eachFilePath([rootDir1, rootDir2], (filePath) => {
              const item = Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                a.textContent.includes(filePath),
              );
              expect(item).toExist();
          ...
          it("shows each file's path, including which root directory it's in", async () => {
            ...
            eachFilePath([rootDir1], (filePath) => {
              ...
              expect(item).toExist();
              expect(item.querySelectorAll("div")[1].textContent).toBe(
                path.join(path.basename(rootDir1), filePath),
              );
            });
            ...
            eachFilePath([rootDir2], (filePath) => {
              ...
              expect(item).toExist();
              expect(item.querySelectorAll("div")[1].textContent).toBe(
                path.join(path.basename(rootDir2), filePath),
              );
            });
          });
          ...
          describe("symlinks on #darwin or #linux", () => {
            ...
            beforeEach(() => {
              ...
              fs.symlinkSync(
                junkFilePath,
                atom.project.getDirectories()[0].resolve("symlink-to-file"),
              );
              fs.symlinkSync(
                junkDirPath,
                atom.project.getDirectories()[0].resolve("symlink-to-dir"),
              );
              fs.symlinkSync(
                brokenFilePath,
                atom.project.getDirectories()[0].resolve("broken-symlink"),
              );

              fs.symlinkSync(
                atom.project.getDirectories()[0].resolve("sample.txt"),
                atom.project.getDirectories()[0].resolve("symlink-to-internal-file"),
              );
              fs.symlinkSync(
                atom.project.getDirectories()[0].resolve("dir"),
                atom.project.getDirectories()[0].resolve("symlink-to-internal-dir"),
              );
              fs.symlinkSync(
                atom.project.getDirectories()[0].resolve(".."),
                atom.project.getDirectories()[0].resolve("symlink-to-project-dir-ancestor"),
              );
            ...
            it("indexes project paths that are symlinks", async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                  a.textContent.includes("sample.txt"),
                ),
              ).toBeDefined();
            });
            ...
            it("includes symlinked file paths", async () => {
              ...
              const sourceSymlink = results.find((a) =>
                a.textContent.includes("symlink-to-internal-file"),
              );
            ...
            it("excludes symlinked folder paths if followSymlinks is false", async () => {
              ...
              expect(
                results.find((a) => a.textContent.includes("symlink-to-dir")),
              ).not.toBeDefined();
              expect(
                results.find((a) => a.textContent.includes("symlink-to-dir/a")),
              ).not.toBeDefined();
              expect(
                results.find((a) => a.textContent.includes("symlink-to-internal-dir")),
              ).not.toBeDefined();
              expect(
                results.find((a) => a.textContent.includes("symlink-to-internal-dir/a")),
              ).not.toBeDefined();
            });
            ...
            it("includes symlinked folder paths if followSymlinks is true", async () => {
              ...
              const results = Array.from(projectView.element.querySelectorAll("li"));
              const sourceSymlink = results.find((a) =>
                a.textContent.includes("symlink-to-internal-dir/a"),
              );
          ...
          describe("socket files on #darwin or #linux", () => {
            ...
            it("does not interfere with ability to load files", async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                  a.textContent.includes("a"),
                ),
              ).toBeDefined();
              expect(
                Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                  a.textContent.includes("some.sock"),
                ),
              ).not.toBeDefined();
            });
          });
          ...
          it("ignores paths that match entries in config.fuzzy-finder.ignoredNames", async () => {
            ...
            expect(
              Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                a.textContent.includes("sample.js"),
              ),
            ).not.toBeDefined();
            expect(
              Array.from(projectView.element.querySelectorAll("li")).find((a) =>
          ...
          it("only shows a given path once, even if it's within multiple root folders", async () => {
            ...
            expect(
              Array.from(projectView.element.querySelectorAll("li")).filter((a) =>
                a.textContent.includes("child-file.txt"),
              ).length,
            ).toBe(1);
          });
          ...
          it("returns all the results if they have the same relative path across multiple root folders", async () => {
            ...
            const results = parseResults(projectView.element).sort((a, b) =>
        ...
        describe("when the project has no path", () => {
          ...
          it("shows an empty message with no files in the list", async () => {
            ...
            expect(projectView.selectListView.refs.emptyMessage.textContent).toBe(
              "Project is empty",
            );
        ...
        describe("toggling", () => {
          describe("when there are pane items with paths", () => {
            ...
            it("shows the FuzzyFinder if it isn't showing, or hides it and returns focus to the active editor", async () => {
              ...
              bufferView.selectListView.refs.queryEditor.insertText(
                "this should not show up next time we toggle",
              );
            ...
            it("lists the paths of the current items, sorted by most recently opened but with the current item last", async () => {
              ...
              expect(atom.workspace.panelForItem(bufferView).isVisible()).toBe(true);
              expect(
                [...bufferView.element.querySelectorAll("li > div.file")].map((e) => e.textContent),
            ...
            it("serializes the list of paths and their last opened time", async () => {
              ...
              let states = _.map(atom.packages.getPackageState("fuzzy-finder"), (path, time) => [
                path,
                time,
              ]);
          ...
          describe("when multiple sessions are opened on the same path", () =>
            it("does not display duplicates for that path in the list", async () => {
              ...
              await filesPromise();
              expect(
                Array.from(bufferView.element.querySelectorAll("li > div.file")).map(
                  (e) => e.textContent,
                ),
              ).toEqual(["sample.js"]);
            }));
      ...
      describe("opening a path into a split", () => {
        it("opens the path by splitting the active editor left", async () => {
          ...
          expect(atom.workspace.getActiveTextEditor().getPath()).toBe(
            atom.project.getDirectories()[0].resolve(filePath),
          );
        });
            ...
            describe("when the colon is followed by more numbers", () => {
              describe("when the filter text has a file path", () => {
                it("opens the selected path to that line number and column", async () => {
                  ...
                  const { filePath } = bufferView.selectListView.getSelectedItem();
                  expect(atom.project.getDirectories()[0].resolve(filePath)).toBe(
                    editor1.getPath(),
                  );
        ...
        describe("when the colon is not followed by numbers", () => {
          ...
          describe("when the filter text doesn't have a file path", () => {
            it("shows an error and does not move the cursor", async () => {
              ...
              expect(bufferView.selectListView.refs.errorMessage.innerText).toEqual(
                "Invalid line number",
              );
      ...
      describe("preserve last search", () => {
        it("does not preserve last search by default", async () => {
          ...
          expect(atom.workspace.panelForItem(projectView).isVisible()).toBe(true);
          bufferView.selectListView.refs.queryEditor.insertText(
            "this should not show up next time we open finder",
          );
        ...
        it("preserves last search when the config is set", async () => {
          ...
          expect(atom.workspace.panelForItem(projectView).isVisible()).toBe(true);
          projectView.selectListView.refs.queryEditor.insertText(
            "this should show up next time we open finder",
          );
      ...
      describe("default file icons", () => {
        it("shows a text icon for text-based formats", async () => {
          ...
          expect(DefaultFileIcons.iconClassForPath(firstResult.dataset.path)).toBe(
            "icon-file-text",
          );
        });
      ...
      describe("Git integration", () => {
        ...
        beforeEach(() => {
          ...
          return new Promise((resolve) => gitRepository.onDidChangeStatuses(resolve));
        });
        ...
        describe("git-status-finder behavior", () => {
          ...
          it("displays all new and modified paths", async () => {
            ...
            expect(gitStatusView.element.querySelectorAll(".status.status-modified").length).toBe(
              1,
            );
        ...
        describe("status decorations", () => {
          ...
          describe("when a modified file is shown in the list", () =>
            it("displays the modified icon", async () => {
              ...
              expect(
                bufferView.element
                  .querySelector(".status.status-modified")
                  .closest("li")
                  .querySelector(".file").textContent,
              ).toBe("a.txt");
            }));
          ...
          describe("when a new file is shown in the list", () =>
            it("displays the new icon", async () => {
              ...
              expect(
                bufferView.element
                  .querySelector(".status.status-added")
                  .closest("li")
                  .querySelector(".file").textContent,
              ).toBe("newsample.js");
            }));
        ...
        describe("when core.excludeVcsIgnoredPaths is set to true", () => {
          ...
          describe("when the project's path is the repository's working directory", () => {
            ...
            it("excludes paths that are git ignored", async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                  a.textContent.includes("ignored.txt"),
                ),
              ).not.toBeDefined();
            });
          });
          ...
          describe("when the .gitignore matches parts of the path to the root folder", () => {
            ...
            it("only applies the .gitignore patterns to relative paths within the root folder", async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll("li")).find((a) =>
                  a.textContent.includes("file.txt"),
                ),
              ).toBeDefined();
            });
          });
      ...
      describe("error handling", () => {
        beforeEach(() => {
          ...
          for (let i = 0; i < 1000; i++) {
            fs.symlinkSync(
              brokenFilePath,
              atom.project.getDirectories()[0].resolve("broken-symlink-" + i),
            );
          }
