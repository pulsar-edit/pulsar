...
module.exports = class ProjectView extends FuzzyFinderView {
  constructor(paths, metricsReporter) {
    ...
    this.disposables.add(
      atom.config.onDidChange("fuzzy-finder.ignoredNames", () => {
        this.reloadPaths = true;
      }),
    );
    this.disposables.add(
  ...
  async populate() {
    const remoteEditors =
      (this.teletypeService && (await this.teletypeService.getRemoteEditors())) || [];
        ...
        if (task) {
          ...
          task.on("load-paths:paths-found", (paths) => {
            ...
            this.selectListView.update({
              loadingMessage: "Indexing project\u2026",
              infoMessage: null,
              loadingBadge: humanize.intComma(pathsFound),
            });
          });
        }
      }
