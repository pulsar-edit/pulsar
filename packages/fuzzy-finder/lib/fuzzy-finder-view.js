...
module.exports = class FuzzyFinderView {
  constructor(metricsReporter) {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: ({ filePath, label, ownerGitHubUsername }) => {
        ...
        atom.ui.fuzzyMatcher.setCandidates(this.nativeFuzzyForResults, [label]);
    ...
    const splitLeft = () => {
      this.splitOpenPath((pane) => pane.splitLeft.bind(pane));
    };
    const splitRight = () => {
      this.splitOpenPath((pane) => pane.splitRight.bind(pane));
    };
    const splitUp = () => {
      this.splitOpenPath((pane) => pane.splitUp.bind(pane));
    };
    const splitDown = () => {
      this.splitOpenPath((pane) => pane.splitDown.bind(pane));
    };
    atom.commands.add(this.selectListView.element, {
      ...
      "fuzzy-finder:invert-confirm": () => {
        this.confirm(this.selectListView.getSelectedItem(), {
    ...
    if (!this.nativeFuzzy) {
      this.nativeFuzzy = atom.ui.fuzzyMatcher.setCandidates(this.items.map((el) => el.label));
  ...
  prefillQueryFromSelection() {
    ...
    if (selectedText) {
      ...
      this.selectListView.refs.queryEditor.setSelectedBufferRange([
        [0, 0],
        [0, textLength],
      ]);
    }
  ...
  projectRelativePathsForFilePaths(filePaths) {
    ...
    if (filePaths !== this.filePaths) {
      ...
      this.projectRelativePaths = this.filePaths.map((filePath) =>
        this.convertPathToSelectViewObject(filePath),
      );
    }
