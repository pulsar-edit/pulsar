      ...
      atom.commands.add("atom-workspace", {
        ...
        "whitespace:save-with-trailing-whitespace": async () => {
          ...
          if (editor) {
            ...
            editor.getPath()
              ? await editor.save()
              : atom.workspace.getCenter().getActivePane().saveActiveItemAs();
            this.ignore = false;
          }
        },
  ...
  handleEvents(editor) {
    ...
    let bufferSavedSubscription = buffer.onWillSave(() => {
      return buffer.transact(() => {
        ...
        if (
          atom.config.get("whitespace.removeTrailingWhitespace", {
    ...
    let editorTextInsertedSubscription = editor.onDidInsertText(function (event) {
      ...
      if (
        ...
        if (
          !atom.config.get("whitespace.ignoreWhitespaceOnlyLines", {
            scope: scopeDescriptor,
          })
        ) {
