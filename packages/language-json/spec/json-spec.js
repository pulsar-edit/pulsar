describe("JSON grammar", () => {
  ...
  it("tokenizes arrays", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "[",
      scopes: [...baseScopes, "punctuation.definition.array.begin.json"],
    });
    ...
    expect(tokens[8]).toEqual({
      value: "]",
      scopes: [...baseScopes, "punctuation.definition.array.end.json"],
    });
  });
  ...
  it("identifies trailing commas in arrays", () => {
    ...
    expect(tokens[2]).toEqual({
      value: ",",
      scopes: [...baseScopes, "invalid.illegal.trailing-array-separator.json"],
    });
    expect(tokens[3]).toEqual({ value: " ", scopes: baseScopes });
  ...
  it("tokenizes objects", () => {
    ...
    expect(tokens[0]).toEqual({
      value: "{",
      scopes: [...baseScopes, "punctuation.definition.dictionary.begin.json"],
    });
    ...
    expect(tokens[14]).toEqual({
      value: "true",
      scopes: [...valueScopes, "constant.language.json"],
    });
    ...
    expect(tokens[22]).toEqual({
      value: '"',
      scopes: [...stringValueScopes, "punctuation.definition.string.begin.json"],
    });
    expect(tokens[23]).toEqual({ value: "bar", scopes: stringValueScopes });
    expect(tokens[24]).toEqual({
      value: '"',
      scopes: [...stringValueScopes, "punctuation.definition.string.end.json"],
    });
    expect(tokens[25]).toEqual({
      value: "}",
      scopes: [...baseScopes, "punctuation.definition.dictionary.end.json"],
    });
  });
  ...
  it("identifies trailing commas in objects", () => {
    ...
    expect(tokens[15]).toEqual({
      value: ",",
      scopes: [...valueScopes, "invalid.illegal.trailing-dictionary-separator.json"],
    });
    expect(tokens[16]).toEqual({ value: " ", scopes: baseScopes });
