...
describe(`${themeName} theme`, () => {
  ...
  it("allows the tab close button to be shown on the left via config", () => {
    ...
    expect(document.documentElement.getAttribute(`theme-${themeName}-tab-close-button`)).toBe(
      "left",
    );
  });
  ...
  it("allows the tree-view headers to be sticky via config", () => {
    ...
    expect(document.documentElement.getAttribute(`theme-${themeName}-sticky-headers`)).toBe(
      "sticky",
    );
  });
