describe("TOML grammar", function () {
  ...
  it("tokenizes comments", function () {
    ...
    expect(tokens[1]).toEqual({
      ...
      scopes: ["source.toml", "comment.line.number-sign.toml"],
    });
  ...
  it("tokenizes strings", function () {
    ...
    expect(tokens[5]).toEqual({
      value: "I am not \\n escaped",
      scopes: ["source.toml", "string.quoted.single.toml"],
    });
    expect(tokens[6]).toEqual({
      value: "'",
      scopes: [
    ...
    expect(tokens[5]).toEqual({
      value: "Equal sign ahead = no problem",
      scopes: ["source.toml", "string.quoted.double.toml"],
    });
    expect(tokens[6]).toEqual({
      value: '"',
...
`);
    expect(lines[0][4]).toEqual({
      ...
      scopes: [
        "source.toml",
        "string.quoted.double.block.toml",
        "punctuation.definition.string.begin.toml",
      ],
    });
    expect(lines[1][0]).toEqual({
      value: "I am a",
      scopes: ["source.toml", "string.quoted.double.block.toml"],
    });
...
`);
    expect(lines[0][4]).toEqual({
      ...
      scopes: [
        "source.toml",
        "string.quoted.single.block.toml",
        "punctuation.definition.string.begin.toml",
      ],
    });
    expect(lines[1][0]).toEqual({
      value: "I am a",
      scopes: ["source.toml", "string.quoted.single.block.toml"],
    });
    expect(lines[2][0]).toEqual({
      value: "string",
      scopes: ["source.toml", "string.quoted.single.block.toml"],
...
`);
    ...
    expect(lines[3][0]).toEqual({
      value: "with\\UaBcDE3F2no escape characters\\naw",
      scopes: ["source.toml", "string.quoted.single.block.toml"],
    });
    expect(lines[4][0]).toEqual({
      value: "'''",
...
`);
    ...
    expect(lines[3][0]).toEqual({
      value: "with no line-continuation\\ \t",
      scopes: ["source.toml", "string.quoted.single.block.toml"],
    });
    expect(lines[4][0]).toEqual({
      value: "aw",
    ...
    (() => {
      ...
      for (let int of ["0xDEADBEEF", "0xdeadbeef", "0xdead_beef"]) {
        ...
        result.push(
          expect(tokens[4]).toEqual({
            value: int,
            scopes: ["source.toml", "constant.numeric.hex.toml"],
          }),
        );
      }
      return result;
  ...
  it("tokenizes octal integers", function () {
    ...
    expect(tokens[4]).toEqual({
      value: "0o755",
      scopes: ["source.toml", "constant.numeric.octal.toml"],
    });
  });
  ...
  it("tokenizes binary integers", function () {
    ...
    expect(tokens[4]).toEqual({
      value: "0b11010110",
      scopes: ["source.toml", "constant.numeric.binary.toml"],
    });
  });
  ...
  it("does not tokenize a number followed by other characters as a number", function () {
    ...
    expect(tokens[4]).toEqual({
      value: "0xdeadbeefs",
      scopes: ["source.toml", "invalid.illegal.toml"],
    });
  });
      ...
      (() => {
        ...
        for (let float of ["inf", "nan"]) {
          ...
          result.push(
            expect(tokens[4]).toEqual({
              value: `${sign}${float}`,
              scopes: ["source.toml", `constant.numeric.${float}.toml`],
            }),
          );
        }
        return result;
  ...
  it("tokenizes offset date-times", function () {
    ...
    expect(tokens[4]).toEqual({
      value: "1979-05-27",
      scopes: ["source.toml", "constant.numeric.date.toml"],
    });
    expect(tokens[5]).toEqual({
      value: "T",
      scopes: ["source.toml", "constant.numeric.date.toml", "keyword.other.time.toml"],
    });
    expect(tokens[6]).toEqual({
      value: "07:32:00",
      scopes: ["source.toml", "constant.numeric.date.toml"],
    ...
    expect(tokens[7]).toEqual({
      value: "Z",
      scopes: ["source.toml", "constant.numeric.date.toml", "keyword.other.offset.toml"],
    });
  ...
  it("tokenizes tables", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "table",
      scopes: ["source.toml", "entity.name.section.table.toml"],
    });
    expect(tokens[2]).toEqual({
      value: "]",
      scopes: [
  ...
  it("tokenizes table arrays", function () {
    ...
    expect(tokens[0]).toEqual({
      ...
      scopes: [
        "source.toml",
        "entity.name.section.table.array.toml",
        "punctuation.definition.table.array.begin.toml",
      ],
    });
    expect(tokens[1]).toEqual({
      value: "table",
      scopes: ["source.toml", "entity.name.section.table.array.toml"],
    });
    expect(tokens[2]).toEqual({
      value: "]]",
      scopes: [
  ...
  it("tokenizes keys", function () {
    ...
    expect(tokens[2]).toEqual({
      value: "=",
      scopes: ["source.toml", "keyword.operator.assignment.toml"],
    });

    ({ tokens } = grammar.tokenizeLine("1key_-34 ="));
    expect(tokens[0]).toEqual({
      value: "1key_-34",
      scopes: ["source.toml", "variable.other.key.toml"],
    });
    expect(tokens[1]).toEqual({ value: " ", scopes: ["source.toml"] });
    expect(tokens[2]).toEqual({
      value: "=",
  ...
  it("tokenizes quoted keys", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "key",
      scopes: ["source.toml", "string.quoted.single.toml", "variable.other.key.toml"],
    });
    expect(tokens[2]).toEqual({
      value: "'",
      scopes: [
    ...
    expect(tokens[1]).toEqual({
      value: "key",
      scopes: ["source.toml", "string.quoted.double.toml", "variable.other.key.toml"],
    });
    expect(tokens[2]).toEqual({
      value: '"',
      scopes: [
