describe("Clojure grammar", function () {
  ...
  it("does not tokenize escaped semicolons as comments", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "clojure",
      scopes: ["source.clojure", "meta.symbol.clojure"],
    });
  });
  ...
  it("tokenizes booleans", function () {
    const booleans = { "constant.language.boolean.clojure": ["true", "false"] };
  ...
  it("tokenizes nil", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "nil",
      scopes: ["source.clojure", "constant.language.nil.clojure"],
    });
  });
    ...
    return (() => {
      ...
      for (let keyfn of Array.from(keyfns)) {
        ...
        result.push(
          expect(tokens[1]).toEqual({
            value: keyfn,
            scopes: ["source.clojure", "meta.expression.clojure", "keyword.control.clojure"],
          }),
        );
      }
      return result;
    ...
    return (() => {
      ...
      for (let mutable of Array.from(mutables)) {
        ...
        result.push(
          expect(tokens[0]).toEqual({
            value: mutable,
            scopes: ["source.clojure", "meta.symbol.dynamic.clojure"],
          }),
        );
      }
      return result;
  ...
  it("tokenizes metadata", function () {
    ...
    expect(tokens[1]).toEqual({
      value: "Foo",
      scopes: ["source.clojure", "meta.metadata.simple.clojure", "meta.symbol.clojure"],
    });
  ...
  it("tokenizes namespaces", function () {
    ...
    expect(tokens[0]).toEqual({
      value: "foo",
      scopes: ["source.clojure", "meta.symbol.namespace.clojure"],
    });
  ...
  const testMetaSection = function (metaScope, puncScope, startsWith, endsWith) {
    ...
    let start = tokens[0],
      adjustedLength = Math.max(tokens.length, 2),
      mid = tokens.slice(1, adjustedLength - 1),
      end = tokens[adjustedLength - 1];
  ...
  it("tokenizes functions in nested sexp", function () {
    ...
    expect(tokens[7]).toEqual({
      value: "baz",
      scopes: ["source.clojure", "meta.expression.clojure", "meta.symbol.clojure"],
    });
