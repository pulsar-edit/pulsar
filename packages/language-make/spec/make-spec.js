...
describe("Makefile grammar", () => {
  ...
  it("parses comments correctly", () => {
    ...
    expect(lines[0][1]).toEqual({
      value: "foo",
      scopes: ["source.makefile", "comment.line.number-sign.makefile"],
    });
    expect(lines[1][0]).toEqual({
      value: "\t",
      scopes: ["source.makefile", "punctuation.whitespace.comment.leading.makefile"],
    });
    ...
    expect(lines[1][0]).toEqual({
      value: "should still be a comment",
      scopes: ["source.makefile", "comment.line.number-sign.makefile"],
    });
  ...
  const testFunctionCall = function (functionName) {
    ...
    expect(tokens[4]).toEqual({
      ...
      scopes: [
        ...
        "support.function." + functionName + ".makefile",
      ],
    });
  };
  ...
  it("parses targets with line breaks in body", () => {
    ...
    expect(lines[1][3]).toEqual({
      ...
      scopes: [
        ...
        "string.interpolated.makefile",
        "meta.scope.function-call.makefile",
        "support.function.basename.makefile",
      ],
    });
  });
  ...
  it("tokenizes variable assignments", () => {
    ...
    for (let operator of Array.from(operators)) {
      ...
      expect(tokens[0]).toEqual({
        value: "SOMEVAR",
        scopes: ["source.makefile", "variable.other.makefile"],
      });
      expect(tokens[1]).toEqual({ value: " ", scopes: ["source.makefile"] });
      expect(tokens[2]).toEqual({
        value: operator,
        scopes: ["source.makefile", "keyword.operator.assignment.makefile"],
      });
    ...
    expect(lines[0][4]).toEqual({
      value: "\\",
      scopes: ["source.makefile", "constant.character.escape.continuation.makefile"],
    });
