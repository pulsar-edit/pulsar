...
module.exports = class TagFinder {
  constructor(editor) {
    // 1. Tag prefix
  ...
  isCursorOnTag() {
    return this.scopesForPositionMatchRegex(
  ...
  findStartEndTags(fullRange = false) {
    ...
    const endPosition = this.editor
      .getLastCursor()
      .getCurrentWordBufferRange({ wordRegex: this.wordRegex }).end;
      ...
      ({ match, range, stop }) => {
        ...
        if (!fullRange) {
          if (range.start.row === range.end.row) {
            ...
            startRange = Range.fromObject([
              range.start.translate([0, prefix.length]),
              [range.start.row, Infinity],
            ]);
          }
        }
  ...
  tagDoesNotCloseInFragment(tags, fragment) {
    ...
    stack = this.parseFragment(
      fragment,
      stack,
      generateTagStartOrEndRegex(escapedTag),
      (s) => s.length >= stackLength || s[s.length - 1] === tag,
    );
