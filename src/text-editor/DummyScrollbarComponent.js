...
class DummyScrollbarComponent {
    ...
    update(newProps) {
        ...
        const shouldFlushScrollPosition = newProps.scrollTop !== oldProps.scrollTop ||
            newProps.scrollLeft !== oldProps.scrollLeft;
        if (shouldFlushScrollPosition)
    ...
    render() {
        const {
            orientation, scrollWidth, scrollHeight, verticalScrollbarWidth, horizontalScrollbarHeight, canScroll, forceScrollbarVisible, didScroll
        } = this.props;
...
exports.DummyScrollbarComponent = DummyScrollbarComponent;
