...
async function buildExcludedPaths(normalizedPath, ignoredNames = []) {
  ...
  console.log(
    "Beginning generation of exclusions",
    normalizedPath,
    ignoredNames,
    performance.now(),
  );
  await new fdir()
    ...
    .exclude((_, dirPath) => {
      ...
      let matches = ignoredNames.some((pattern) =>
        minimatch(dirPath, pattern, { matchBase: true }),
      );
  ...
  console.log(
    "Generated exclusions in",
    end - start,
    "ms",
    "with time spent minimatching:",
    _totalTimeSpentMinimatching,
  );
...
process.title = `Lumine file watcher worker (NSFW) [PID: ${process.pid}]`;
