...
# Lumine. It's meant to run in Lumine's CI and almost certainly won't do
...
# that it points to `lumine.sh` internally and runs _that_ when invoked instead
...
#   executable; modify it to invoke our `lumine.sh` script instead.
...
# to run
#
#   ./Lumine.AppImage --wait foo.txt
#
# and keep an eye on the terminal window after Lumine launches. It should not
...
cd binaries
LUMINE_APPIMAGE="$(ls *.AppImage | xargs)"
echo "Existing binary is ${LUMINE_APPIMAGE}."
chmod +x "${LUMINE_APPIMAGE}"

echo "Extracting ${LUMINE_APPIMAGE} to Lumine.AppDir…"
"./${LUMINE_APPIMAGE}" "--appimage-extract"
# Will extract to `squashfs-root`. Let's rename it just for sanity.
mv "squashfs-root" "Lumine.AppDir"

# Move the `AppImage` to a new filename because we'll be replacing it soon.
mv "${LUMINE_APPIMAGE}" "${LUMINE_APPIMAGE%.AppImage}.old.AppImage"
...
echo "Building new AppImage at: binaries/${LUMINE_APPIMAGE} with ARCH value: ${APPIMAGETOOL_ARCH}…"
ARCH="${APPIMAGETOOL_ARCH}" ./appimagetool --appimage-extract-and-run "binaries/Lumine.AppDir" "binaries/${LUMINE_APPIMAGE}"
chmod a+x "binaries/${LUMINE_APPIMAGE}"
echo "…done building AppImage."

# Clean up! Remove the old AppImage and the temporary directory.
echo "Removing temporary Lumine.AppDir and old AppImage…"
rm -rf "binaries/Lumine.AppDir"
rm -f "binaries/${LUMINE_APPIMAGE%.AppImage}.old.AppImage"
